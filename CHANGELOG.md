...
### Added
- **PA2-AUTO-004 : garde CI signalant les PR sans ID `PA2-*`.** Le protocole de claim `PA2-AUTO-011` verifie deja les collisions *quand* une PR reference un ID `PA2-*`, mais aucun garde ne signalait une PR livrant un ticket du backlog sans reference explicite (perte de tracabilite pour le rapport hebdo `PA2-AUTO-005` et pour la revue). Nouveau script `dev-hub/tools/check-pr-has-pa2-id.sh <pr_title> <pr_body_file>` : recherche un ID `PA2-<AREA>-<NNN>` dans le titre/la description ; si absent, tolere un opt-out explicite (`PA2: none` dans la description) ou un prefixe conventionnel `docs:`/`chore:` (avec scope optionnel) dans le titre ; sinon emet `::warning` (non bloquant par defaut, exit 0). Variable de repo `PA2_ID_CHECK_STRICT=1` disponible pour passer en `::error`/exit 1 une fois le taux de faux positifs verifie sur des PR reelles. Nouveau job `pa2-id-check` dans `.github/workflows/plan-action2-claim-guard.yml` (memes evenements que le job de collision de claim existant). Verifie manuellement : 6 cas (ID present, absent sans opt-out, prefixe `docs:`, prefixe `chore(scope):`, opt-out `PA2: none`, mode strict) tous corrects ; `shellcheck`/`actionlint` propres.
- **PA2-AUTO-006 : template PR enrichi (surfaces, contrat API, risques residuels).** `.github/PULL_REQUEST_TEMPLATE.md` ne demandait ni les surfaces touchees, ni un rappel explicite de compatibilite du contrat API, ni les risques residuels assumes — information utile perdue a chaque PR alors que le corps de PR sert de source pour le rapport hebdo (`PA2-AUTO-005`) et pour le garde de claim (`PA2-AUTO-011`/`PA2-AUTO-004`, qui lisent le titre/la description). Ajout d'un champ `PA2 ticket ID` en tete (ou `PA2: none`), d'une section "Surfaces touchees", d'une section "Contrat API (si applicable)" (case a cocher compatibilite + doc OpenAPI, ou N/A), d'une section "Risques residuels", d'une case CHANGELOG dans la checklist qualite existante, et rendu la section captures/GIF obligatoire pour tout changement UI (case N/A sinon). Aucune section existante supprimee.
- **PA2-AUTO-008 : regles agents juniors (choix de ticket, anti-duplication, review).** Section dediee ajoutee dans `AGENTS.md` (juste apres "Regles obligatoires") : comment choisir un ticket dans `docs/PLAN_ACTION2/02_BACKLOG_ATOMIQUE.md`/les issues GitHub sans dupliquer un travail deja livre (y compris fermer une issue deja resolue mais restee ouverte, cf. `PA2-I18N-007`/issue #1098), rappel condense du protocole de claim deja documente dans `01_MODE_EXECUTION_MULTI_AGENT.md`, et quand demander une review humaine explicite (RBAC, paiement/avance, secrets/infra, contrat API deja consomme) plutot que de se fier uniquement aux checks automatises.
- **PA2-AUTO-009 : nettoyage controle des branches distantes stale.** `dev-hub/tools/repository-hygiene-report.ps1` listait deja les branches fusionnees/non fusionnees mais ne definissait aucun seuil de "stale" (anciennete) et n'offrait aucune suppression, meme manuelle. Nouveau script bash `dev-hub/tools/cleanup-stale-branches.sh [--days N] [--delete] [--include-unmerged] [--yes] [--no-fetch]` : classe chaque branche distante (hors `main`/`HEAD`) en fusionnee+stale (candidate sure), stale-mais-non-fusionnee (revue manuelle requise), ou active ; dry-run par defaut (liste seulement) ; `--delete` supprime uniquement les branches fusionnees+stale sauf `--include-unmerged` explicite ; confirmation interactive sauf `--yes` ; ne tourne jamais automatiquement en CI (outil manuel/local, coherent avec l'absence de modification automatisee d'etat de branche partagee sans decision humaine). Verifie sur un dépôt bare de test avec 3 branches synthetiques (fusionnee+stale, non-fusionnee+stale, recente) : classement correct, dry-run n'altere rien, `--delete` seul ne supprime que la branche fusionnee+stale, `--delete --include-unmerged` supprime ensuite la branche non-fusionnee restante ; `shellcheck` propre. Corrige au passage un piege `git for-each-ref` reel : le nom court du symref `refs/remotes/origin/HEAD` s'affiche comme `origin` (pas `origin/HEAD`), desormais explicitement exclu.
