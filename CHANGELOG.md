...
### Added
- **test: tests HR/Attendance sur les vraies migrations (F-13b, #1593/#1606).** Migration des 13 tests `Feature/Attendance` + 5 tests `Feature/Contracts` de `CreatesMvpSchema` (schéma manuel) vers `RefreshTenantDatabase` (vraies migrations public+tenant) ; `PunchPhotoTest` ne crée plus les tables SmartAttendance en double. Comptage global Feature : **65 fichiers** sur `RefreshTenantDatabase` — les 5 tests `Feature/SmartAttendance` (géofencing, sessions, mode config, validation manager, isolation tenant) sont migrés également (schéma manuel `CreatesSmartAttendanceSchema` supprimé). Rapport de dérive archivé dans `docs/audits/DRIFT_F13B_2026-08-09.md` (40 tables restantes, plan par module, règle de retrait au fil de l'eau).
