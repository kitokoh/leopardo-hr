...
### Added
- **test(mobile): géofencing F-21 (#1551) — tests GeofenceService (dans/hors/horizon/config invalide) + doc `docs/mobile/F21_FIABILITE_MOBILE.md`.** `leopardo_employee/test/features/smart_attendance/geofence_service_test.dart` (9 tests) : transitions enter/exit, premier check sans événement, horizon exact, GPS désactivé, distance Haversine (~111 km/degré), reset. La doc consolide l'état réel F-21 : file hors-ligne + sync avec dédoublonnage `external_event_id` (1er pointage gagne), géofencing par site (`SmartAttendanceConfig`), anti-fraude (photo, double check-in bloqué, horodatage serveur).
