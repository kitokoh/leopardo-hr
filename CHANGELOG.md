...
### Security
- **fix(security): secure /api/v1/metrics, realign CORS origins, add global security headers (issues #1466, #1468, #1469).** (1) `GET /api/v1/metrics` (public since inception) exposed `php_version`/`laravel_version`, cache/queue/db drivers and tenant/employee counts to anonymous callers â€” version fingerprinting + business intelligence; now gated behind `auth:super_admin_api` (`api/routes/api.php`), health endpoints untouched for Render deploy hooks. (2) `config/cors.php` allowed `leopardo-hr.vercel.app` (returns 404) while the real vitrine origin `https://gestionemployer-backend.vercel.app` (see `front/web/.env.local.example`) was missing, so any future direct browser API call from the vitrine would be silently CORS-blocked; added the real origin + `https://admin.leopardo-rh.com`. (3) New `App\Http\Middleware\SecurityHeaders` registered globally (`bootstrap/app.php`) sending `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: none` and `Strict-Transport-Security` (HTTPS only) on every response â€” the API previously sent no security headers. New tests: `tests/Feature/Security/MetricsAccessTest.php` (anon 401, super_admin 200, health stays public), `tests/Feature/Security/SecurityHeadersTest.php` (headers present, HSTS HTTPS-only), CORS regression in `CorsAndTrustedProxyTest`.
