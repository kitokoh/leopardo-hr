...
### Fixed
- **fix(api): DDD cleanup — dead Expense Domain/Application/Infrastructure code, wrong ExpenseClaimPolicy model, duplicated absence + notifications routes (issue #1414).** `Modules/Expense/{Domain,Application,Infrastructure}` (8 files: `ExpenseClaim`, `ExpenseItem`, `ExpenseRepositoryInterface`, `ExpenseNotDraftException`, `CreateExpenseClaim`, `SubmitExpenseClaim`, `CreateExpenseDTO`, `ExpenseService`) were unreferenced dead code — the actual `ExpenseClaimController` has always consumed `Planning\Domain\Models\ExpenseClaim`. Fixed `AuthServiceProvider::Gate::policy(ExpenseClaim::class)` to register `ExpenseClaimPolicy` on the real model (`Planning\...\ExpenseClaim`) instead of the dead one. Removed `ExpenseServiceTest.php` (tested dead code only). Removed 6 duplicated absence routes from `routes/modules/rh.php` (source of truth: `routes/modules/absence.php` which also has `/proof`, `POST approve/reject`). Removed duplicated `GET /notifications` from `rh.php` (source of truth: `dashboard.php`). Removed unused `AbsenceController` import from `rh.php`. Documented the Expense facade derogation in `api/ARCHITECTURE.md` (same pattern as the existing PA2-ARCH-002 Absence derogation).
