...
### Added
### Added
- **security: rétention biométrique S-1 (#1661) — purge automatique des templates expirés.** Nouvelle commande `biometric:purge-expired` (par tenant via `TenantManager::withinTenant`, options `--company`, `--months`, `--dry-run`) : nullifie les références (`employees.biometric_*_reference_path`, `biometric_enrollment_requests.*_reference_path`), désactive les flags, supprime les fichiers physiques et trace dans `audit_logs` (`biometric_templates_purged` + compteurs + base légale). Règle de rétention : `max(fin de contrat, consentement)` + 24 mois (config `security.biometric.retention_months`). Politique `POLITIQUE_RETENTION_DOCUMENTS.md` étendue (§2bis biométrie, base légale consentement Loi 18-07). Tests : `BiometricPurgeExpiredCommandTest` (purgé/non-purgé/contrat différé/dry-run/idempotent/tenant-scoped/enrôlements/audit).
