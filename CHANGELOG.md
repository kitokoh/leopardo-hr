...
### Added
### Added
- **test(qualité): couverture Payroll ≥ 80 % + gate bloquante (S-4, #1664).** Fix du CI rouge main (`EndOfContractApiTest` : `employees.position` inexistante sur les vraies migrations → `position_id` + `Position`). Nouveaux tests : `PayrollCalculatorEdgeCasesTest` (unit — prorata, heures sup golden, indemnité congés, solde de tout compte, `computeComponentAmount` privé via réflexion) et `PayrollCalculatorRunEdgeTest` (feature — garde run verrouillé F-11, rebuild des bulletins, prorata contrat en cours de mois, entrées de travail pointage/congés). Gate `payroll-ci.yml` : `continue-on-error` retiré, échec si coverage < 80 %.
