...
### Fixed
- **fix(security): alerte automatisée secrets exposés dans l'historique (A-2, #1680).** Nouveau workflow hebdo `secret-history-scan.yml` (lundi 06:30 UTC, + dispatch) : TruffleHog sur **tout** l'historique depuis la racine, **non bloquant**, commente l'issue tracker #1472 si des secrets sont détectés. Inventaire `docs/security/HISTORIQUE_SECRETS.md` : Redis Upstash (critique, rotation #1472), Neon (#1601), placeholders — valeurs jamais citées (convention #1614).
