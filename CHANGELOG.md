...
### Added
- **docs/security/POLITIQUE_RETENTION_DOCUMENTS.md + commande `audit:purge` (issue #1474).** La matrice RGPD référençait `audit:purge --older-than=24months` sans que la commande existe (point PARTIEL « limitation de conservation »). Nouvelle commande `audit:purge` (défaut 24 mois, planifiée hebdomadairement dans `routes/console.php`), tests unitaires (`PurgeAuditLogsCommandTest`), et politique de rétention documentée par catégorie de document (logs 24 mois, paie 10 ans, contrats 5 ans, biométrie fin d'emploi, recrutement 2 ans, etc.) avec bases légales FR/DZ/MA et mécanismes de purge existants ou à construire.
- **dev-hub/tools/check-openapi-coverage.sh (issue #1473).** Script de comparaison routes (`php artisan route:list`) vs chemins documentés dans `api/openapi.yaml` (normalisation des paramètres) : mesure actuelle **176 chemins non documentés** malgré la fermeture de #1409 — le script rend l'écart mesurable et CI-able.
