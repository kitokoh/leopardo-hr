...
### Added
- **test: audit du pattern PendingCommand lazy (A-1, #1679).** Vérifié dans le code Laravel : `assertExitCode()`/`assertSuccessful()`/`assertFailed()` n'enregistrent que l'expectation, la commande tourne au `__destruct` — seuls les `PendingCommand` STOCKÉS dans une variable sont dangereux (assertions DB avant exécution). Audit complet des 42 usages de `$this->artisan()` dans `api/tests/` : 39 sûrs (chaînage sans affectation), 3 fichiers corrigés (`PurgeAuditLogsCommandTest::test_noop_*` — `run()` explicite ; `GdprAnonymizeEmployeeTest` — `run()` explicite). Convention documentée : `docs/GESTION_PROJET/CONVENTIONS_TESTS.md`.
