...
### Fixed
- **ci(openapi): garde de couverture routes -> operations (issue #1473).** `dev-hub/tools/check-openapi-route-coverage.py` compare les routes Laravel (prefixes imbriques) aux operations documentees dans `api/openapi.yaml` ; gaps connus dans `openapi-coverage-allowlist.txt` (289 entrees) ; toute nouvelle route non documentee et non allowlistee fait echouer `openapi-ci.yml`.
