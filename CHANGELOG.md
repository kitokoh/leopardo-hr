...
### Fixed
- **feat(payroll): cycle de cloture 2 etapes + verrouillage operationnel (F-11, #1541).** Statut `locked` + colonnes `locked_by`/`locked_at` (migration tenant avec resolution dynamique d'enum, alignee sur le schema de test) ; `PayrollClosingService` (validateRh/lock/unlock) avec transitions conditionnelles atomiques, audit trail en vrais tableaux, exceptions DomainException (423) ; `PayrollRunLockedException` mappee. Porte aussi F-05 (prorata/HS/absences), F-07 (conge indemnite 1/10e vs maintien), F-08 (solde de tout compte), F-10 (journal + declaration CNAS CSV), F-20 (lien pointage->paie) avec golden tests DZ.
