...
### Fixed
- **Onboarding: `artisan migrate` never created the tenant schema (71 missing tables), silently breaking every local onboarding path (issue #1393).** Leopardo RH's multi-tenant model needs both the `public` and `shared_tenants` PostgreSQL schemas populated, which the custom `leopardo:migrate` Artisan command already handled correctly (used by `api/docker-entrypoint.sh` in prod and `api/tests/RefreshTenantDatabase.php` in CI) — but the 4 local onboarding entry points (`Makefile`'s `install`/`migrate`/`migrate-fresh`, `DEVELOPMENT.md`'s Quick Start and Backend sections, `scripts/bootstrap.sh`, and `docker-compose.yml`'s header comment, plus `.devcontainer/devcontainer.json`) all called bare `artisan migrate`/`migrate:fresh`, which only reads `database/migrations/` at the project root (2 of 96 migration files) and never touches `database/migrations/{public,tenant}/`. A contributor following the docs literally got an API answering on `/health` that 500s on every business route (employees, payroll, attendance...). Switched all 5 entry points to `leopardo:migrate` (with `--fresh`/`--seed` where the original command used the equivalent native flags), added a `DEVELOPMENT.md` "Why `leopardo:migrate`" paragraph explaining the two-schema model, and added a new CI job `onboarding-smoke` (`.github/workflows/tests.yml`, gated on a new `onboarding` path filter in `.github/paths-filters.yml`) that replays the exact documented Quick Start (`docker compose up -d --build` + `leopardo:migrate --seed --demo`) and asserts a demo manager can log in and fetch a non-empty employee list, so a regression back to bare `artisan migrate` fails CI instead of only breaking new contributors.
