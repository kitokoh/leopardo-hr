...
### Security
- **fix(mobile): centralize remaining hardcoded hex colors into AppColors (CI PA2-MOB-011 guard).** `validate-mobile-color-tokens.ps1` flagged pre-existing `Color(0x...)` literals in `leopardo_core` (`glass_card`, `glass_tile`, `startup_gate`) whenever the package was touched. Moved them to new `AppColors` tokens (`mobileDarkGlass`, `mobileDarkSurfaceLow`, `mobileMint`, `mobileLavender`) or existing ones (`mobileDarkBg`, `rh`) â€” zero visual change, guard now green.
- **fix(mobile): never activate MockInterceptor in release builds (issue #1470, audit T14 2026-04-22).** `ApiClient` activated the mock interceptor whenever `API_BASE_URL=mock` was compiled in, with no `kReleaseMode` guard â€” a release build accidentally built with `--dart-define=API_BASE_URL=mock` would silently serve fake data in production. Guarded with `!kReleaseMode` so the mock path is tree-shaken from release bundles entirely.
...
### Fixed
- **fix(mobile): repair 23 misnamed widget constructors from #1431 - employee/manager apps compile again (issue #1482).** The glassmorphism PR renamed classes `_XGlassCard` -> `_XCard` but left the constructors (and some call sites) with the old name; Dart requires constructor = class name, so `const _XGlassCard(...)` parsed as a const *method* and every screen instantiating one failed to compile. Renamed all 23 occurrences across 14 files in `leopardo_employee`/`leopardo_manager` (home, modules hub, payroll lists, personal space, profile, smart attendance, tasks, user auth). The `mobile-apps-ci.yml` Analyze matrix, which never ran on #1431, now validates these apps.
### Fixed
