...
### Added
- **feat(security): journalisation des accès aux données sensibles (Spec S-2, #1662).** Les lectures sensibles (téléchargement/consultation de bulletins, journal de paie, export de run, export bancaire, solde de tout compte, certificat, exports du dashboard) sont tracées dans `audit_logs` via `DataAccessAuditLogger` (metadata.category `hr_data_access`, qui/quoi/quand). Échantillonnage configurable `audit.data_access.sample_rate` (défaut 1.0, déterministe par acteur+action — config/audit.php). Nouvelle commande `audit:sensitive-report` (`--days/--since/--company/--json/--csv`) pour le rapport périodique. Tests : `SensitiveDataAccessAuditTest` (un test par ressource sensible, taux 0, déterminisme, rapport).
