...
### Security
- **fix(deps): resolve remaining npm advisories â€” postcss â‰¥ 8.5.23 + brace-expansion/js-yaml (issues #1464, #1465).** (1) `postcss` resolved â‰¤ 8.5.22 in `front/web` and `front/web-offline` while GHSA-fxqj-rqcc-2cmp requires â‰¥ 8.5.23 (Dependabot alerts #59/#63, moderate) â€” overrides bumped `^8.5.10` â†’ `^8.5.23`, lockfiles regenerated (resolves 8.5.26). (2) `npm audit fix` in `front/web`, `front/web-offline`, `front/admin-dashboard` resolved the remaining high advisories `brace-expansion` (GHSA-mh99-v99m-4gvg / GHSA-rgw5-rvv9-x895, DoS) and `js-yaml` (CVE-2026-59870, quadratic CPU) â€” `brace-expansion` 5.0.9, `js-yaml` 3.15.1. All three `npm audit --package-lock-only` now report **0 vulnerabilities** (previously: front/web 4, front/web-offline 3, front/admin-dashboard 1).
...
### Fixed
- **fix(admin-dashboard): repair broken Tailwind classes `glass-card/N` (pre-existing main break, issue #1481).** The build failed with `Cannot apply unknown utility class 'glass-card/50'` â€” `glass-card` is a `@layer components` class, not a Tailwind color utility, so the `/opacity` modifier is invalid in Tailwind v4. All 13 views using `glass-card/{5,10,20,50,70,90}` (plain, `hover:`, `dark:`) now use the equivalent explicit `bg-white/N dark:bg-slate-900/N` classes â€” zero visual change (the modifier was expressing exactly those opacities). This was blocking every PR touching `front/admin-dashboard` (also #1459, #1460). Two further pre-existing main breaks found while validating the build: `src/style.css` had `@config` between two `@import`s (postcss parse error) and `src/stores/auth.js` (from #1299) used TypeScript type annotations in a `.js` file (`Empty parenthesized expression`) â€” both fixed; plus lint blockers in 7 views (`no-irregular-whitespace` NBSP, C1 control chars, mojibake emoji icons in EdgeNodesView/RecruitmentView from a bad encoding pass) â€” `vite build` and `eslint` (0 errors) now green locally.
