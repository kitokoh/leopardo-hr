...
### Added
- **feat(security): alerte automatisée secrets dans l'historique git (Spec A-2, #1680).** Nouveau workflow hebdo `secret-history-scan.yml` (lundi 06:00 UTC + dispatch manuel) : scan TruffleHog de TOUT l'historique (`--since-commit` racine, v3.96.0 épinglée, mêmes exclusions que secret-scan.yml) en job d'INFORMATION non bloquant (`continue-on-error`) avec résumé + rappel de l'inventaire. `docs/security/HISTORIQUE_SECRETS.md` : inventaire des secrets exposés (Redis Upstash #1472, Neon #1601, clés Google #1467), gravité, plan de purge coordonné (action humaine). Un résultat de scan absent de l'inventaire = nouveau secret → traiter immédiatement.
