...
### Added
- **feat(payroll): durcissement paie (Spec S-3, #1663).** (a) Migration additive `social_contributions.effective_from` NOT NULL (backfill `created_at`/repli, garde `information_schema`) — plus de 500 potentiel ; validation store `effective_to after:effective_from` conservée/couverte par test API. (b) `PayrollCycleService::safeEmployeeBalance` propage désormais l'erreur (500 explicite + `Log::error` contextuel) au lieu du fallback silencieux à valeurs fabriquées. (c) Migrations additives de rattrapage : `2026_08_09_000006` (geo_auto sur `attendance_logs.method` — ENUM `ADD VALUE`/CHECK, idempotente) et `2026_08_09_000007` (`company_id` uuid sur `zkteco_devices` + `kiosk_announcements`) — commentaires de renvoi dans les fichiers d'origine 000205/000003. (d) `SET search_path TO public` retiré de `public/0001` (requêtes qualifiées `public.*`). Tests : `PayrollHardeningTest` (NOT NULL, geo_auto accepté, uuid kiosque, validation store, propagation d'erreur).
