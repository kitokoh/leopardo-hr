...
### Fixed
- **fix(ci): correctifs des echecs pre-existants sur main (2026-08-09).** (1) `PayrollAnomalyService::detectDuplicateSlips()` utilisait `having('slip_count', '>', 1)` - PostgreSQL interdit de referencer un alias de SELECT dans HAVING (SQLSTATE 42703) : remplace par `havingRaw('COUNT(*) > 1')`. (2) `ExpenseClaimResource` exposait `amount` (colonne inexistante) au lieu de `total_amount` - le test `ExpenseClaimWorkflowTest` echouait. (3) `check-i18n-diff.js` : les diffs de reformatage Dart produisaient des faux positifs - le scanner ignore desormais les litteraux deja presents dans le hunk retire (re-enroulage), tout en continuant de bloquer les vraies nouvelles chaines (self-test verifie).
- **feat(payroll): kit de demo DZ (F-06/F-23) + benchmark de cloture (F-12).** `DemoDzPayrollSeeder` (opt-in : `php artisan db:seed --class=DemoDzPayrollSeeder`) : entreprise DZ fictive, ~30 employes, 4 grilles salariales + composants, 3 cycles de paie clotures (locked). `dev-hub/tools/benchmark-payroll-closing.sh` : mesure du temps de cloture (seuil F-12 < 30 min). Guide de demo : `docs/demo/DEMO_SCRIPT_15MIN.md`.
- **chore(mobile): formatage Dart des 6 apps (218 fichiers) - drift Flutter stable 3.44.9.** `dart format` applique via CI sur leopardo_core/employee/hr/manager/marketing/platform_admin (purement formatage).
- **docs(audit): rapport complet `docs/audits/AUDIT_NEO_2026-08-09.md`** - verification des audits precedents, etat du CI main, correctifs livres, restes (issues #1585-#1589).
