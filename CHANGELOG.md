...
### Added
- **Docs audit P2: extend `docs/README.md` central index with 16 previously unlisted top-level `docs/` subfolders.** A documentation audit (2026-07-29, read-only GitHub token) found the central `docs/README.md` index referenced only 20 of the 34 top-level `docs/` subfolders — a dev agent following it as the sole entry point would never discover `GESTION_PROJET/` (33 files: runbooks, QA/deployment reports), `audits/`, `validation/` (40+ release-readiness reports), `specifications/` (the mandatory new-module spec location per `AGENTS.md`'s golden rule), `vision/`, `CONTEXT/`, `GUIDES/`, `GTM/`, `STRATEGIE_COMMERCIALE/`, `commercial/`, `admin/`, `ai/`, `api-mock-data/`, `edge-sync/`, `kiosk/`, `external-audits/`, or `assets/`. Added 3 new sections ("Produit & Stratégie" extended with 4 entries; new "Pilotage projet, audits & validation" with 7 entries; new "Domaines techniques additionnels" with 6 entries), each pointing to the folder and a one-line description sourced from that folder's own README where one exists. Verified: cross-check confirming every top-level `docs/` subfolder (except `archive/`, already covered via its `archive/PLAN_ACTION/` subpath entry) now has a matching link in `docs/README.md`; relative-link scan (`os.path.exists` on every parsed Markdown link) found 0 broken links. No code change.
