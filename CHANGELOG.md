...
### Added
- **feat(security): journalisation des accès aux données sensibles (Spec S-2, #1662).** `DataAccessAuditLogger::recordSensitive()` trace les lectures sensibles (bulletins PDF, documents de paiement, journal de paie CSV, exports bancaires, fin de contrat, certificat) avec la catégorie `sensitive_data_access` et un échantillonnage configurable (`config/audit.php` → `SENSITIVE_ACCESS_SAMPLING`, 0.0 = désactivé, 1.0 = 100 %). Branché dans 5 contrôleurs : `PaySlipController@downloadPdf`, `PaymentDocumentController@download`, `PayrollRunController@journal`, `BankExportController@show/download`, `EndOfContractController@settlement/certificate`. Nouvelle commande `audit:sensitive-report` (`--days`, `--company`) : comptage par action/tenant sur la fenêtre + échantillon des dernières traces. Tests : `SensitiveAccessAuditTest` (un test par ressource sensible, échantillonnage 0, rapport).
