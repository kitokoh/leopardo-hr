...
## 2026-05-02 - Compound Accessibility Labels
**Learning:** Screen readers perform better when related information (like a status icon and its label, or several metric fields) is announced as a single logical unit.
**Action:** Use `Semantics(container: true, label: '...')` to group related widgets, and `ExcludeSemantics` on the inner children to avoid redundant or fragmented announcements.
