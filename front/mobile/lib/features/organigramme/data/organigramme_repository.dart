...
class OrgNode {
  ...
  factory OrgNode.fromJson(Map<String, dynamic> json) {
    ...
    return OrgNode(
      id: json['id'] as int,
      name: (json['name'] ?? '${json['first_name'] ?? ''} ${json['last_name'] ?? ''}').toString().trim(),
...
class OrganigrammeRepository {
  ...
  Future<List<OrgNode>> getOrgChart() async {
    ...
    return items.map((e) => OrgNode.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<List<OrgNode>> getDepartmentHierarchy(int departmentId) async {
    final response = await apiClient.dio.get('/departments/$departmentId/hierarchy');
    final items = response.data['data'] as List;
    return items.map((e) => OrgNode.fromJson(e as Map<String, dynamic>)).toList();
  }
}
