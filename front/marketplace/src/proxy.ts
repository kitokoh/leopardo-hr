 ...
 * routes HTML sont rendues dynamiquement (le root layout déclare
 * `dynamic = "force-dynamic"`, #8022). `x-nonce` sert aux composants
 * serveur qui rendraient un script inline explicite (aucun aujourd'hui).
 */
...
export const config = {
  matcher: [
    // Toutes les routes HTML — JAMAIS les assets statiques de build (la CSP
    // n'a de sens que sur du HTML ; aucune route /api dans cette app).
