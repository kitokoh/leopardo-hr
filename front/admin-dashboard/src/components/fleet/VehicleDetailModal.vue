...
async function fetchDetail() {
  ...
  error.value = ''
  try {
    const res = await api.get(`/vehicles/${props.vehicleId}`, { _skipAuthRedirect: true })
    detail.value = res.data?.data || res.data

    try {
      const alertsRes = await api.get(`/vehicles/${props.vehicleId}/alerts`, { _skipAuthRedirect: true })
