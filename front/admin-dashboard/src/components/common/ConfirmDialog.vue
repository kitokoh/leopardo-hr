    ...
    >
      <div class="w-full max-w-md glass-card p-6">
        ...
        <div class="mt-4 flex justify-end gap-2">
          <button class="btn-secondary" @click="cancel">{{ cancelLabel || $t('time.cancel', 'Annuler') }}</button>
          <button class="btn-danger" @click="confirm" :disabled="busy">
            {{ busy ? (busyLabel || $t('time.inProgress', 'En cours…')) : (confirmLabel || $t('time.confirm', 'Confirmer')) }}
          </button>
        </div>
      </div>
    </div>
