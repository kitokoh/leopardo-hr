<template>
  <div class="space-y-8 animate-fade-in max-w-6xl">
    ...
    <div class="glass-card p-6">
      ...
      <!-- Issue #1872: compliance banner - payroll rules confidence level (localized) -->
      <div
        v-if="confidenceLevel"
        class="mt-6 rounded-2xl border p-4"
        :class="confidenceBannerClass"
        role="alert"
      >
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p
              v-if="simResult.compliance_warning && confidenceMessage !== simResult.compliance_warning"
              class="mt-1 text-xs text-slate-500 dark:text-slate-400"
            >
              {{ simResult.compliance_warning }}
            </p>
          </div>
        </div>
      </div>
...
// Issue #1872 — bandeau de conformité : niveau de confiance des règles pays
// renvoyé par POST /payroll/simulate (confidence_level + compliance_warning).
...
const confidenceLevel = computed(() => simResult.value?.confidence_level || null)
