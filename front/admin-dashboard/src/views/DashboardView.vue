        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-white/5 dark:divide-white/5">
            ...
            <tbody class="divide-y divide-white/5 bg-transparent dark:divide-white/5 dark:bg-transparent">
              <tr v-for="item in priorityCompanies" :key="item.id" class="hover:bg-white/5 dark:hover:bg-slate-900/5 transition-colors">
                <td class="whitespace-nowrap px-6 py-4">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <div class="text-sm font-bold text-slate-900 dark:text-white">{{ item.company?.name || item.name }}</div>
                      <div class="text-[10px] font-medium text-slate-400 uppercase tracking-widest">{{ item.company?.slug || item.slug || '—' }}</div>
                    </div>
                  </div>
                </td>
                <td class="whitespace-nowrap px-6 py-4">
                ...
                <td class="whitespace-nowrap px-6 py-4 text-sm font-black text-slate-950 dark:text-white">
                  {{ formatCurrency(item.subscription?.mrr ?? item.mrr_eur, item.subscription?.currency || 'EUR') }}
                </td>
                <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                  <router-link :to="`/companies/${item.company?.id ?? item.id}`" class="text-emerald-500 hover:text-emerald-600 dark:text-emerald-400 font-bold">
                    Détails
                  </router-link>
                </td>
              </tr>
      ...
      <section class="card">
        ...
        <div class="divide-y divide-white/5 dark:divide-white/5">
          <div v-for="request in pendingCompanyRequests" :key="request.id" class="p-6 transition-colors hover:bg-white/5 dark:hover:bg-slate-900/5">
            <div class="flex items-start justify-between">
              <div class="space-y-1">
                <h3 class="text-sm font-bold text-slate-950 dark:text-white uppercase tracking-tight">{{ request.company_name || request.name }}</h3>
                <p class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ request.email || request.manager_email }}</p>
