...
/**
 * TravelCrudTable (BC-24, #7634) — tableau CRUD générique config-driven de
 * l'espace gérant Travel. Consomme l'API tenant `/travel/*` via `apiFetch`
 * (réponses paginées Laravel enveloppées `{ data: [...] }`).
 *
 * Calqué sur `RestaurantCrudTable` mais adapté aux FormRequests du module
 * TravelAgency : sélecteurs à valeurs numériques (villes, lignes…), cases à
 * cocher booléennes (`is_terminal`, `is_stopover`), champs optionnels omis
 * du payload quand ils sont vides (les règles `sometimes`/`nullable` du
 * backend rejettent les chaînes vides), validation locale des champs requis
 * et messages localisés via les clés IMBRIQUÉES `travel.*`.
 */
...
export type TravelCrudOption = { value: string | number; label: string };

export type TravelCrudField = {
  name: string;
  label: string;
  type: 'text' | 'number' | 'select' | 'checkbox' | 'date' | 'time';
  required?: boolean;
  /** Options du select ; `numeric` force la coercition en nombre. */
  options?: TravelCrudOption[];
  numeric?: boolean;
...
export type TravelCrudColumn = {
  key: string;
  label: string;
  render?: (row: Record<string, unknown>) => ReactNode;
};

export type TravelRowAction = {
  key: string;
  label: string;
  onClick: (row: Record<string, unknown>) => void;
};

export type TravelCrudConfig = {
  /** Endpoint relatif, ex. `/travel/stations`. */
  endpoint: string;
  /** Query string de liste (sans `?`), ex. `per_page=500`. */
  listQuery?: string;
  title: string;
  columns: TravelCrudColumn[];
  fields: TravelCrudField[];
  searchKeys: string[];
  searchPlaceholder?: string;
  canCreate?: boolean;
  canDelete?: boolean;
  rowActions?: TravelRowAction[];
};
...
export function TravelCrudTable({ config }: { config: TravelCrudConfig }) {
  ...
  const submit = async () => {
    ...
    if (missing) {
      setFormError(
        `${missing.label} — ${t(locale, 'travel.form.required', 'Ce champ est obligatoire.')}`,
      );
      return;
    }
    setSaving(true);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={config.searchPlaceholder ?? t(locale, 'travel.search.placeholder', 'Rechercher…')}
            aria-label={config.searchPlaceholder ?? t(locale, 'travel.search.placeholder', 'Rechercher…')}
              ...
              {config.fields.map((field) => (
                <label key={field.name} className="block text-sm">
                  ...
                  {field.type === 'select' ? (
                    ...
                    >
                      <option value="">
                        {t(locale, 'travel.form.selectPlaceholder', '— Sélectionner —')}
                      </option>
