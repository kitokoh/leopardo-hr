...
/**
 * Primitives UI du module Caméras BC-19 (#7425) — copies minimales des tokens
 * glass du portail, sur le modèle de `(dashboard)/fuel/_components/fuel-ui.tsx`
 * (aucune dépendance à un module non mergé, aucun hex hors palette).
 */
  ...
  return (
    <label className="inline-flex items-center gap-2 text-sm font-medium text-slate-700">
      <input
        {...props}
        type="checkbox"
        className={`h-4 w-4 rounded border-slate-300 text-emerald-700 focus:ring-emerald-500 ${props.className ?? ''}`}
      />
      {label}
    </label>
  );
...
export function Modal({ open, title, onClose, closeLabel, children }: ModalProps) {
  if (!open) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={title}>
      <button type="button" aria-label={closeLabel} className="absolute inset-0 bg-slate-950/40 backdrop-blur-sm" onClick={onClose} />
...
export function Notice({
  ...
  return (
    <div role="status" className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${tones[tone]}`}>
      {children}
    </div>
  );
}
