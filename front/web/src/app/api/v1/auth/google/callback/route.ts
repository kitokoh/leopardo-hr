...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const backendUrl = new URL(`${resolveBackendBaseUrl()}/auth/google/callback`);
  ...
  if (backendResponse.status >= 400) {
    ...
    const errorCode = backendError === 'UNKNOWN_ACCOUNT' ? 'google_no_account' : 'google_auth_failed';
  ...
  if (!token) {
    return NextResponse.redirect(new URL('/auth/login?error=google_no_account', request.url));
  }
