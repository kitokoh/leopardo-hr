...
export default async function GuidesPlanningEmployesLayout({
  children,
}: {
  ...
  const guide = getPageMetadata('guidePlanningEmployes', lang);
    ...
    <>
      <BreadcrumbJsonLd
        items={[
          ...
          { name: guide.title, url: localizedUrl('/guides/planning-employes', lang) },
        ]}
      />
      {children}
    </>
