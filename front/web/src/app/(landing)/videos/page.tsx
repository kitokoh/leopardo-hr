 ...
 * Contenu localisé ×4 locales (issue #3248) : videosPageCopy.
 */

export default function VideosPage() {
  const { isDark, toggleDarkMode } = useDarkMode();
  const { locale } = useVitrineLocale();
  const copy = videosPageCopy[locale] ?? videosPageCopy.fr;
  useScrollReveal();
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'dark bg-slate-950' : 'bg-white'}`}>
      ...
      <HeroSection
        ...
        ctaPrimary={{ text: copy.hero.ctaPrimary, href: '/demo' }}
        ctaSecondary={{ text: copy.hero.ctaSecondary, href: '/signup' }}
        badge={{ text: copy.hero.badge, icon: <MonitorPlay className="w-3 h-3" /> }}
      />
          ...
          >
            <div className="px-6 py-4 bg-slate-900 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-bold text-white text-lg">{copy.demo.title}</h2>
                  <p className="text-slate-400 text-sm">{copy.demo.description}</p>
                </div>
              </div>
            </div>
            <video
              ...
              aria-label={copy.demo.ariaLabel}
            >
      ...
      <section className="py-16 px-4 bg-transparent dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-center mb-12 max-w-2xl mx-auto">
            {copy.upcoming.subtitle}
          </p>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {copy.upcoming.videos.map((video, i) => (
              <motion.div
                key={video.title}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="relative aspect-video bg-slate-100 dark:bg-slate-900 flex items-center justify-center">
                  ...
                  <span className="absolute bottom-3 right-3 bg-black/70 text-white text-xs px-2 py-1 rounded-md flex items-center gap-1">
                    <Clock className="w-3 h-3" />{copy.upcoming.badge}
                  </span>
      ...
      <CTASection
        ...
        primaryCta={{ text: copy.cta.primary, href: '/demo' }}
        secondaryCta={{ text: copy.cta.secondary, href: '/pricing' }}
      />
