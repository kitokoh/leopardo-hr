/**
 * Catalogue inline ×4 (fr/en/tr/ar) des libellés du survey de solutions.
 *
 * Vit dans `data/` (chemin exempté par la garde PA2-I18N-014, comme
 * case-studies.ts / pricing.ts) : ce fichier EST le mécanisme i18n du
 * wizard « Je suis restaurateur », pas des chaînes hors catalogue.
 * Les clés `label_key`/`reason_key` du backend y sont résolues.
 *
 * @see docs/architecture/RESTAURANT_SOLUTION_SURVEY.md
 */

export type SurveyLabelLocales = 'fr' | 'en' | 'tr' | 'ar';

export type SurveyLocalized = { fr: string; en: string; tr: string; ar: string };

export const SOLUTION_LABELS: Record<string, SurveyLocalized> = {
