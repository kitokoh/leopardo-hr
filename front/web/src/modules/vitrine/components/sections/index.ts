// Hero Section
export { HeroSection, type HeroSectionProps } from './HeroSection';

// Problem & Solution Components
export { ProblemSection, type ProblemSectionProps, type ProblemItem } from './ProblemSection';
export { SolutionSection, type SolutionSectionProps, type SolutionFeature } from './SolutionSection';

// Feature Components
export { FeatureCard, type FeatureCardProps } from './FeatureCard';
export { FeaturesSection, type FeaturesSectionProps } from './FeaturesSection';

// Pricing Components
export { PricingCard, type PricingCardProps } from './PricingCard';
export { PricingSection, type PricingSectionProps } from './PricingSection';

// Testimonial Components
export { TestimonialCard, type TestimonialCardProps } from './TestimonialCard';
export { TestimonialsSection, type TestimonialsSectionProps } from './TestimonialsSection';

// Case Study Components
export { CaseStudyCard, type CaseStudyCardProps } from './CaseStudyCard';
export { CaseStudiesSection, type CaseStudiesSectionProps } from './CaseStudiesSection';

// FAQ Section
export { FAQSection, type FAQSectionProps, type FAQItem } from './FAQSection';

// CTA Section
export { CTASection, type CTASectionProps } from './CTASection';

// Blog Components
export { BlogCard, type BlogCardProps } from './BlogCard';
export { BlogGrid, type BlogGridProps } from './BlogGrid';
export { BlogArticle, type BlogArticleProps } from './BlogArticle';

// Social Proof Components
export { SocialProofMetrics, type SocialProofMetricsProps, type MetricItem } from './SocialProofMetrics';
export { SocialProofTestimonial, type SocialProofTestimonialProps } from './SocialProofTestimonial';
export { SocialProofCases, type SocialProofCasesProps, type MiniCaseItem } from './SocialProofCases';

// Product Screenshots
export { ProductScreenshots, type ProductScreenshotsProps, type ScreenshotItem } from './ProductScreenshots';
