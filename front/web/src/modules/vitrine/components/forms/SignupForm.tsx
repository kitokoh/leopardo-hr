...
// #7489 — parcours minimal demandé par le propriétaire : l'e-mail et le nom de
// l'espace, puis le code reçu par e-mail. Le choix du PROFIL (entreprise ou
// indépendant) et l'étape « outils + métier » ont été retirés du tunnel : le
// profil est désormais la PREMIÈRE question de l'entretien de préparation
// (#7493), posée à chaud dans l'espace — « êtes-vous seul(e) ou une équipe ? ».
// Aucun champ « rôle / taille / téléphone / profil » n'est affiché avant
// l'entrée dans l'espace.
...
export function SignupForm({
  ...
  // Multi-step state
  // #7489 — le tunnel s'ouvre directement sur les coordonnées (e-mail + nom de
  // l'espace) : plus d'écran de choix de profil avant la création du compte.
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      // #7489 — le profil (entreprise/indépendant) n'est plus demandé à
      // l'inscription : il devient la première question de l'entretien de
      // préparation (#7493). `company_type` reste optionnel côté API
      // (nullable) — l'absence de valeur laisse le backend appliquer son
      // défaut, et la réponse de l'entretien fera autorité ensuite.
      // Le créateur du compte EST le fondateur : le rôle n'est pas demandé
    ...
    <Card className={`p-6 md:p-8 ${className}`}>
      ...
      <AnimatePresence mode="wait">
        ...
        {currentStep === 'form' && (
          ...
          >
            {/* #7489 — le badge était porté par l'écran de profil ; il ouvre
                désormais le premier écran du tunnel (e-mail + nom d'espace). */}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {/* #7853 — le nom d'entreprise n'est plus demandé : l'inscription
                  se fait par e-mail seul. Le serveur dérive un nom provisoire
                  depuis l'e-mail, et le nom définitif est demandé dans
                  l'entretien de préparation (#7493, question `company_name`). */}
