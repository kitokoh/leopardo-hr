...
void main() {
  group('AppStrings — i18n ×4 (BC-19 DEVICE, #7426)', () {
    ...
    test('chaque type d\'événement et statut d\'alerte du contrat API a un libellé', () {
      const types = ['motion', 'person', 'vehicle', 'line_crossing', 'tamper'];
      ...
      for (final locale in AppStrings.supportedLocales) {
        ...
        for (final type in types) {
          expect(app.keys, contains('eventType_$type'), reason: locale);
        }
        for (final severity in severities) {
          expect(app.keys, contains('severity_$severity'), reason: locale);
        }
        for (final status in statuses) {
          expect(app.keys, contains('alertStatus_$status'), reason: locale);
        }
      }
    });
  });
