...
/// Authentification email/mot de passe (pattern leopardo_travel_agent /
/// leopardo_accounting, fichier préfixé « cameras_ » pour le garde
/// anti-duplication core<->apps #7652).
///
/// `/cameras/*` exige `auth:sanctum` + `tenant` + `module.cameras`
/// (companies.features.cameras) + `api.manager` : l'app est réservée aux
/// responsables/managers du tenant (RBAC porté par le compte). Aucun
/// identifiant RTSP ne transite ni n'est stocké côté mobile (#7426, AC 4) :
/// seul le token Sanctum est persisté dans le stockage sécurisé.
class CamerasAuthRepository {
  final ApiClient apiClient;
  final SecureStorage storage;
  final AppPreferences preferences;

  CamerasAuthRepository(this.apiClient, this.storage, this.preferences);
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    ...
    // Hydrate depuis /auth/me pour récupérer manager_role + features
    // (hasCamerasModule) + capabilities.
    try {
