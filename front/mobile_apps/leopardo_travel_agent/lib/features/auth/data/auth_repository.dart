...
/// Authentification email/mot de passe (pattern leopardo_accounting).
///
/// `/travel/*` exige `auth:sanctum` + permissions `travel.*` : l'app est
/// réservée aux agents/vendeurs du tenant (RBAC porté par le compte).
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    final response = await apiClient.requestWithRetry(
      ...
      data: {
        'email': email,
        'password': password,
        'device_name': 'Leopardo Travel Agent',
      },
      isLoginRequest: true,
