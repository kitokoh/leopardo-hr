...
import 'package:leopardo_core/features/attendance_geo/data/attendance_geo_repository.dart';
import 'package:leopardo_core/features/attendance_geo/data/models/geo_attendance_session.dart';

final attendanceGeoRepositoryProvider = Provider<AttendanceGeoRepository>((ref) {
  return AttendanceGeoRepository(ref.watch(apiClientProvider));
});
...
final pendingGeoSessionsProvider =
    FutureProvider.autoDispose<List<GeoAttendanceSession>>((ref) async {
      return ref.watch(attendanceGeoRepositoryProvider).getPendingSessions();
    });

/// Stats du dashboard Smart Attendance.
final smartAttendanceDashboardProvider =
    FutureProvider.autoDispose<Map<String, dynamic>>((ref) async {
      return ref.watch(attendanceGeoRepositoryProvider).getDashboardStats();
    });
