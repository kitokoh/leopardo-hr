...
void main() {
  ...
  test(
    'syncNow() short-circuits while offline and never calls the API',
    () async {
  ...
  test(
    'syncNow() with nothing pending succeeds with zero sent/failed',
    () async {
  ...
  test(
    'syncNow() pushes pending items and marks them synced on success',
    () async {
      service.debugSetModeForTesting(SyncMode.cloud);
  ...
  test(
    'syncNow() marks items failed for retry on a transient push error',
    () async {
      service.debugSetModeForTesting(SyncMode.cloud);
      await db.insertAttendanceLog(
        LocalAttendanceLogsCompanion.insert(
