...
/// Issue #3006 (T166) : l'app Marketing n'avait aucun flux d'authentification —
/// `/marketing/*` exige `auth:sanctum` + `api.manager:marketing,principal` →
/// 401 systématique sans guidance. Ce repository porte le login email/mot de
/// passe (pattern leopardo_manager/leopardo_hr), sans Google ni inscription
/// (hors périmètre marketing).
class AuthRepository {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    final response = await apiClient.requestWithRetry(
      '/auth/login',
      method: 'POST',
      data: {'email': email, 'password': password, 'device_name': 'Marketing App'},
      isLoginRequest: true,
    ...
    // (la réponse /auth/login ne les expose pas).
    try {
  ...
  Future<Map<String, dynamic>?> checkAuth() async {
    ...
    try {
      ...
      // #2736 : ne supprimer le token que sur un 401 explicite — une erreur
      // réseau/timeout ne doit pas détruire la session.
  ...
  /// (les réponses d'erreur ne sont pas des maps).
