...
- Depuis v4.16.250, `GET /api/v1/kiosks/{deviceCode}/announcements` doit rester tolerant aux tenants historiques dont la table `kiosk_announcements` existe mais sans toutes les colonnes modernes (`company_id`, `is_active`, `starts_at`, `expires_at`, `priority`, `created_at`). Utiliser `Schema::getColumnListing`/`hasColumn` + `DB::table` defensif plutot que le scope Eloquent complet pour eviter les 500 Render. Sans `company_id`, retourner `data=[]` pour proteger l'isolation tenant.
