...
- Depuis v4.16.231, `EmployeeController@index` doit garder son `select` compatible avec `EmployeeResource` sans supposer que toutes les colonnes optionnelles existent deja sur tous les environnements. Utiliser `employeeIndexColumns()` + `Schema::hasColumn`, et garder `EmployeeResource::employeeAttribute()` tolerant aux attributs absents. Si `GET /api/v1/employees` retourne 500 en production, verifier d'abord les champs partiellement charges/colonnes optionnelles avant de toucher aux policies.
