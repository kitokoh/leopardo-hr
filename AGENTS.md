...
- En login shared PostgreSQL, ne pas supposer que `Company` se resout via le `search_path` courant : si un schema tenant contient une table `companies`, il peut masquer `public.companies`. `AuthService` doit recharger l'entreprise depuis `public.companies` avant de conclure `COMPANY_NOT_FOUND`.
