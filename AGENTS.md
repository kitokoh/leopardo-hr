...
- Depuis v4.16.250, la recette API lancement par profil passe par `dev-hub/tools/launch-api-profile-smoke.ps1`. Fournir les tokens via variables d'environnement (`LEOPARDO_MANAGER_TOKEN`, `LEOPARDO_EMPLOYEE_TOKEN`, `LEOPARDO_PLATFORM_ADMIN_TOKEN`, `LEOPARDO_KIOSK_DEVICE_CODE`, `LEOPARDO_KIOSK_TOKEN`) et ne jamais les commiter. La creation entreprise de test est volontairement gardee par `-IncludePlatformProvisioning`.
