...
- Depuis v4.16.153, la distribution mobile main est portee par `mobile-distribute-main.yml` (`push: main` + `paths: front/mobile_apps/**`, design CodeQL #1396 : `github.sha` verifie, pas de `workflow_run`). Depuis la dedup #3756 (734d0c3ba), `mobile-distribute.yml` ne porte PLUS de declencheur `push: branches` : il reste manuel (`workflow_dispatch`) + tags staging/prod. Ne pas re-ajouter `branches:` sur `mobile-distribute.yml` (double build main) et ne pas casser `mobile-distribute-main.yml` (la garde `validate-mobile-plan29.ps1` exige son `branches: [main]` + `paths: front/mobile_apps/**`, #4391). Garder `FIREBASE_EMPLOYEE_ANDROID_APP_ID`, `FIREBASE_MANAGER_ANDROID_APP_ID`, `FIREBASE_PLATFORM_ADMIN_ANDROID_APP_ID` et les fichiers `google-services.json` synchronises ; `deploy-main.yml` peut sauter une app non prete, mais le workflow mobile manuel doit echouer si l'app demandee n'est pas correctement configuree.
