...
- **Mobile employee : les écrans Formation et Véhicules appelaient `/me/training-enrollments` et `/me/vehicles` inexistants (404).** La règle : tout repository mobile doit être cross-checké contre `php artisan route:list` ; l'audit `audit_mobile.py`-style (endpoints vs routes réelles) est disponible dans le workspace de la session.
