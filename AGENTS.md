...
- Depuis v4.16.175, `front/mobile_apps/` est la source canonique des apps mobiles de lancement. `front/mobile/` reste une app historique de maintenance : ne pas y ajouter de nouvelle fonctionnalite produit employee/manager/platform admin. Les workflows `Legacy Mobile CI - Flutter`, `Legacy Mobile Flutter` et les assets de release `leopardo-rh-legacy-*` ne valident que ce perimetre historique ; les apps store passent par `mobile-apps-ci.yml` et `mobile-distribute.yml`.
