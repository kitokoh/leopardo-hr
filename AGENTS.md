...
- Spec S-2 (#1662) : les lectures sensibles passent par `DataAccessAuditLogger::recordSensitive()` (catégorie `sensitive_data_access`, échantillonnage `SENSITIVE_ACCESS_SAMPLING`). Toute nouvelle route de lecture sensible (bulletin, export, journal, certificat, fin de contrat) doit l'utiliser ; les noms d'action restent ≤ 30 caractères (colonne `audit_logs.action`).
