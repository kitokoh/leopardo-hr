...
- Le secret GitHub `FIREBASE_SERVICE_ACCOUNT_JSON` est optionnel mais recommande pour le readback Firebase strict. Quand il est absent, les workflows gardent le fallback `FIREBASE_TOKEN` et n'echouent pas si le listing Firebase CLI est indisponible apres upload reussi.
