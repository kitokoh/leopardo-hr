...
- Le smoke staging des vrais comptes demo vit dans `dev-hub/tools/staging-demo-auth-smoke.sh` et reste desactive par defaut. L'activer uniquement avec `demo_auth_smoke=true` ou `STAGING_DEMO_AUTH_SMOKE=true` + secrets `STAGING_*`; en production normale `/api/v1/demo-users` peut retourner 403 volontairement.
