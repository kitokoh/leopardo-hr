...
- [x] T1. Claim : issue #3727 self-assignée + branche `fix/3727-belongs-to-company-fail-closed` (protocole #2400)
...
- [x] T5. Opt-outs explicites pré-tenant : `WebAuthController::login`, `DemoLoginController`, `RequestTrialSignup::findExistingManager` + plateforme (LaunchReadiness, webhooks, ActivateCompany, provisioning)
- [x] T6. Test `TenantScopeFailClosedTest` (8 scénarios : fail-closed HTTP, contrainte explicite OK, console legacy, config off)
