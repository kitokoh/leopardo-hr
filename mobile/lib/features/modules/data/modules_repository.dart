...
class ModulesRepository {
  ...
  Future<List<Evaluation>> listEvaluations({int perPage = 30}) async {
    final response = await _apiClient.dio.get('/evaluations', queryParameters: {
      'per_page': perPage,
    });
    final items = response.data['data'] as List;
    return items
        .map((item) => Evaluation.fromJson((item as Map).cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<Evaluation> createEvaluation({
    ...
    final response = await _apiClient.dio.post('/evaluations', data: {
      'employee_id': employeeId,
      'period': period.trim(),
      if (score != null) 'score': score,
      if (strengths != null && strengths.trim().isNotEmpty) 'strengths': strengths.trim(),
      if (improvements != null && improvements.trim().isNotEmpty) 'improvements': improvements.trim(),
      if (overallComment != null && overallComment.trim().isNotEmpty) 'overall_comment': overallComment.trim(),
    });
    return Evaluation.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }

  Future<Evaluation> submitEvaluation(int evaluationId) async {
    final response = await _apiClient.dio.put('/evaluations/$evaluationId/submit');
    return Evaluation.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }

  Future<Evaluation> acknowledgeEvaluation(int evaluationId) async {
    final response = await _apiClient.dio.put('/evaluations/$evaluationId/acknowledge');
    return Evaluation.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }
  ...
  Future<List<SalaryAdvance>> listSalaryAdvances({int perPage = 30}) async {
    final response = await _apiClient.dio.get('/salary-advances', queryParameters: {
      'per_page': perPage,
    });
    final items = response.data['data'] as List;
    return items
        .map((item) => SalaryAdvance.fromJson((item as Map).cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<SalaryAdvance> createSalaryAdvance({
    ...
    final response = await _apiClient.dio.post('/salary-advances', data: {
      'amount': amount,
      'reason': reason.trim(),
      'repayment_months': repaymentMonths,
    });
    return SalaryAdvance.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }

  Future<SalaryAdvance> approveSalaryAdvance(int advanceId, {String? decisionComment}) async {
    final response = await _apiClient.dio.put('/salary-advances/$advanceId/approve', data: {
  ...
  Future<SalaryAdvance> rejectSalaryAdvance(int advanceId, {String? decisionComment}) async {
    final response = await _apiClient.dio.put('/salary-advances/$advanceId/reject', data: {
      if (decisionComment != null && decisionComment.trim().isNotEmpty)
        'decision_comment': decisionComment.trim(),
    });
    return SalaryAdvance.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  ...
  Future<List<PayrollRecord>> listPayrolls({int perPage = 30}) async {
    final response = await _apiClient.dio.get('/payrolls', queryParameters: {
      'per_page': perPage,
    });
    final items = response.data['data'] as List;
    return items
        .map((item) => PayrollRecord.fromJson((item as Map).cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<PayrollRecord> createPayroll({
    ...
    final response = await _apiClient.dio.post('/payrolls', data: {
      'employee_id': employeeId,
    ...
    return PayrollRecord.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }
  ...
  Future<AppNotification> markNotificationRead(int notificationId) async {
    ...
    return AppNotification.fromJson((response.data['data'] as Map).cast<String, dynamic>());
  }
