...
function FaqAccordion({ item, index }: { item: ReturnType<typeof getFaqItems>[number]; index: number }) {
    ...
    >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <p className="pb-6 text-slate-500 dark:text-slate-400 leading-relaxed max-w-3xl">{item.answer}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 gsap-reveal">
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white mb-6 tracking-tight">
            {copy.faq.title}{' '}
            <span className="bg-gradient-to-r from-sky-500 to-blue-500 bg-clip-text text-transparent">{copy.faq.titleHighlight}</span>
          </h2>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900/80 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 px-8 divide-slate-200 dark:divide-slate-800">
          {faqItems.map((item, index) => (
            <FaqAccordion key={`${item.question}-${index}`} item={item} index={index} />
          ))}
        </div>
      </div>
    </section>
