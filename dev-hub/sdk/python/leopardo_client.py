...
class LeopardoClient:
    ...
    def post_accounting_payment_orders(self, **kwargs):
        """Créer un ordre de virement depuis un run de paie validé"""
        return self.request("POST", "/accounting/payment-orders", **kwargs)

    def get_accounting_payment_orders_by_order(self, **kwargs):
        """Détail d'un ordre de virement"""
        return self.request("GET", "/accounting/payment-orders/{order}", **kwargs)

    def post_accounting_payment_orders_by_order_execute(self, **kwargs):
        """Exécuter un ordre de virement (référence banque + date = rapprochement)"""
        return self.request("POST", "/accounting/payment-orders/{order}/execute", **kwargs)

    def post_accounting_payment_orders_by_order_prepare(self, **kwargs):
        """Préparer un ordre de virement (génération de l'export banque)"""
        return self.request("POST", "/accounting/payment-orders/{order}/prepare", **kwargs)
