...
Add-Check "Mobile" "Flutter project present" (Test-PathExists "front/mobile_apps/leopardo_core/pubspec.yaml") "front/mobile_apps/leopardo_core/pubspec.yaml" "Restore the canonical mobile_apps Flutter project."
Add-Check "Mobile" "Mobile test suite present" ($mobileAppsTests -ge 20) "$mobileAppsTests Dart tests under front/mobile_apps" "Add widget/model tests for principal mobile flows (front/mobile_apps)."
...
Add-Check "Operations" "Production ops readiness governance present" ((Test-PathExists "dev-hub/tools/validate-production-ops-readiness.ps1") -and (Test-PathExists "docs/validation/PRODUCTION_OPS_READINESS_REPORT_2026_06_01.md") -and (Test-PathExists "docs/deployment/DEPLOYMENT_GUIDE.md")) "Ops readiness guard, report and deployment guide" "Restore production ops readiness governance before launch."
...
Add-Check "CI/CD" "Core CI workflows present" ((Test-PathExists ".github/workflows/tests.yml") -and (Test-PathExists ".github/workflows/web-ci.yml") -and (Test-PathExists ".github/workflows/mobile-apps-ci.yml") -and (Test-PathExists ".github/workflows/openapi-ci.yml")) "tests, web, mobile, openapi workflows" "Restore required workflow files."
...
Add-Check "Governance" "Launch workflow contracts present" ((Test-PathExists "dev-hub/tools/launch-workflow-contracts.json") -and (Test-PathExists "dev-hub/tools/validate-launch-workflows.ps1")) "Launch workflow manifest and validator" "Restore launch workflow governance before marketing launch."
Add-Check "Governance" "Launch API profile smoke present" ((Test-PathExists "dev-hub/tools/launch-api-profile-smoke.ps1") -and (Test-PathExists ".github/workflows/launch-api-profile-smoke.yml") -and (Test-PathExists "docs/validation/LAUNCH_API_PROFILE_SMOKE_2026_06_06.md")) "Plan 72 API profile smoke script, manual workflow and report" "Restore Plan 72 API profile smoke before opening broad marketing traffic."
