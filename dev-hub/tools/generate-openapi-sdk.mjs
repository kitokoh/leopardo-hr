...
function generateDevHubOpenApiMirror() {
  const source = readFileSync(specPath, "utf8");

  const banner = [
    "# GENERATED FILE — do not edit manually.",
    "#",
    "# Mirror of api/openapi.yaml (the canonical API contract), generated by",
    "# dev-hub/tools/generate-openapi-sdk.mjs so external integrators browsing",
    "# dev-hub/ (per ARCHITECTURE.md, the folder \"dediee aux developpeurs et",
    "# integrateurs externes\") always see the real, full API surface instead of",
    "# a stale hand-written stub.",
    "#",
    "# To update: change api/openapi.yaml, then run:",
    "#   node dev-hub/tools/generate-openapi-sdk.mjs",
    "# CI (openapi-ci.yml) fails the build if this file drifts from the source.",
    "",
  ].join("\n");
...
writeOrCheck("dev-hub/openapi/v1.yaml", generateDevHubOpenApiMirror());

console.log(`${checkOnly ? "Checked" : "Generated"} ${operations.length} OpenAPI operations for JavaScript and Python SDKs, and refreshed dev-hub/openapi/v1.yaml.`);
