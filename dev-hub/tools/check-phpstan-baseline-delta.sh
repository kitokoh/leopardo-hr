...
#
# Context: docs/archive/PLAN_ACTION2/ (historique) — plan de réduction baseline (api/phpstan-baseline.neon) (created for
...
if [ ${#touched_modules[@]} -eq 0 ]; then
  echo "This diff only touches legacy app/Http, app/Services or non-module code — no per-module baseline to check (voir api/phpstan-baseline.neon et sa réduction progressive)."
  exit 0
fi
...
if [ "$VIOLATIONS" -gt 0 ]; then
  ...
  echo "Fix the new finding instead of adding it to the baseline, ou réduisez la baseline (api/phpstan-baseline.neon)."
  exit 1
fi
