...
|---|---|---|---|---|---|
| R1 | Service en salle complet (GJ-RESTO-01) | caisse → table → commande dine_in → article → submit → cuisine start/ready → serve → bill → pay cash → close table → close caisse | Flux complet sans 409/422 ; écart de caisse = 0 (fonds + encaissements) | ⬜ | lien CI du test `RestaurantGoldenJourneyTest` |
