# Templates de Prospection — Leopardo

Templates prets a l'emploi pour la prospection LinkedIn et WhatsApp.

---

## LinkedIn — Message InMail DRH PME

### Template 1 : Accroche pointage

> Bonjour [Prenom],
>
> Je vois que [Societe] compte [X] employes — vous gerez probablement encore le pointage et les conges manuellement ?
>
> Leopardo est une plateforme SaaS qui automatise la paie, le pointage et les conges pour les PME du Maghreb. Nos clients reduisent le temps paie de 80%.
>
> Seriez-vous ouvert a une demo de 15 min cette semaine ?

### Template 2 : Accroche paie

> Bonjour [Prenom],
>
> Les erreurs de cotisations CNAS/CNSS coutent cher aux PME. Leopardo calcule automatiquement les baremes fiscaux DZ/MA/SN/TR avec bulletins PDF conformes.
>
> Accepteriez-vous une demo gratuite ? Pas d'engagement.

### Template 3 : Follow-up

> Bonjour [Prenom],
>
> Je me permets de revenir vers vous — nous avons recemment aide [Cas Client] a reduire de 80% leur temps de traitement paie.
>
> Voici un temoignage rapide : [lien cas client]
>
> 15 min suffisent pour voir si ca peut vous aider aussi.

---

## WhatsApp — Templates de relance

### Template 1 : Premier contact

> Bonjour [Prenom] ! 👋
> Je suis [Nom] de Leopardo. On aide les PME a automatiser la paie et les conges.
> Seriez-vous disponible pour un appel de 10 min cette semaine ?
> 📊 Nos clients gagnent 2 jours/mois sur l'admin RH.

### Template 2 : Apres demo

> Bonjour [Prenom] !
> Suite a notre demo, voici le recapitulatif :
> ✅ Paie automatisee (baremes [Pays])
> ✅ Pointage digital + app mobile
> ✅ Dashboard RH avec KPI
> 💰 A partir de 3 USD/employe/mois
> Voulez-vous demarrer un essai gratuit de 14 jours ?

### Template 3 : Relance douce

> Bonjour [Prenom],
> Je voulais m'assurer que vous aviez pu jeter un oeil a Leopardo.
> On a un nouveau cas client dans votre secteur : [lien]
> N'hesitez pas si vous avez des questions ! 🙂

---

## Pricing a mentionner

| Plan | Prix | Inclus |
|------|------|--------|
| Starter | 3 USD/employe/mois | Pointage + Conges + Bulletins |
| Pro | 5 USD/employe/mois | + Recrutement + Formations + Chat IA |
| Enterprise | Sur devis | + Multi-site + API + SSO + Support dedie |

Essai gratuit 14 jours sur tous les plans. Pas de carte bancaire requise.

---

## Cibles prioritaires

| Pays | Secteurs | Taille |
|------|----------|--------|
| Algerie (DZ) | BTP, Alimentation, Services | 20-200 employes |
| Maroc (MA) | Services, Industrie, Sante | 20-500 employes |
| Senegal (SN) | Transport, Agriculture, Tech | 20-200 employes |
| Cote d'Ivoire (CI) | Tech, Services, Banque | 15-100 employes |
| Cameroun (CM) | Industrie, Agriculture | 20-200 employes |
| Turquie (TR) | Textile, Services | 50-500 employes |
