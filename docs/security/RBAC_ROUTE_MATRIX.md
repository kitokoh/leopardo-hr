...
| Announcements `/announcements*` (PA2-COMM-004) | RW any audience | RW any audience | RW own department/team only | - | RW own team only | read scoped (company/own dept/direct/self-authored), no create | `AnnouncementController::authorizeAudience` fails closed: only P/RH can target `company`; DEPT/SUP may only target their own department/direct reports; delete is author-only or P/RH. Index scopes to company-wide + actor's own department/direct-target/self-authored rows. |
