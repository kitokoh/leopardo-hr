...
| CRM client `/crm/*` (leads, opportunities, pipelines, stages, tasks, activities) | RW | RW | - | - | - | - | CRM client tenant (CRM-V0-07/08, #5711/#5712) : groupe `throttle:api, auth:sanctum, token.refresh, tenant, throttle:api-plan` + `api.manager:principal,rh` (`routes/modules/crm.php`) — distinct du CRM commercial Platform (`/platform/*`). Policies dédiées (`app/Policies/Crm/*`) : propriétaire/assigné en lecture-édition sur leur lead/tâche, suppression manager-only, timeline activities append-only (aucune route de modification). Isolation tenant BelongsToCompany fail-closed #3727 + garde 404 cross-tenant ; enums/statuts/tris allowlistés (422). Tests : `CrmApiPolicyValidationTest`, `CrmApiContractTest`. |
