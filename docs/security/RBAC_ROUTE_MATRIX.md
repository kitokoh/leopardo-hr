...
| Payroll engine `/salary-*`, `/tax-slabs`, `/social-contributions`, `/payroll-runs`, `/bank-exports` | RW | R | - | RW | - | self payslips only | Self-service payslip routes are `/me/pay-slips*`; manager routes must not leak across tenant/FK chains. F-11 (#1541) : `POST /payroll-runs/{run}/validate`, `lock`/`unlock` (raison obligatoire) — réservés principal/comptable (`api.manager:principal,comptable`) ; `cancel` refusé si verrouillé ; audit trail `payroll_run_validated/locked/unlocked` (`PayrollRunClosingApiTest`). F-10 (#1540) : `GET /payroll-runs/{run}/journal` (CSV, bulletins validés + totaux) — isolation tenant (`PayrollJournalApiTest`). F-20 (#1550) : `GET /payroll-runs/{run}/anomalies` (rapport pré-clôture lecture seule) (`PayrollAttendanceAnomalyApiTest`). |
