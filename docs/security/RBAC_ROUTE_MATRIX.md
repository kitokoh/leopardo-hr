...
| Payroll engine `/salary-*`, `/tax-slabs`, `/social-contributions`, `/payroll-runs`, `/bank-exports` | RW | R | - | RW | - | self payslips only F-08 (#1538) : `GET /employees/{employee}/end-of-contract` (solde de tout compte) + `GET /employees/{employee}/certificate-of-employment` (certificat PDF) — managers, isolation tenant. Tests : `EndOfContractApiTest`. | Self-service payslip routes are `/me/pay-slips*`; manager routes must not leak across tenant/FK chains. |
