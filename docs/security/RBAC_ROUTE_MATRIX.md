...
| Admin cockpit paie (`/api/v1/admin/*`) | `/admin/public-holidays`, `/admin/tax-slabs`, `/admin/social-contributions`, `/admin/rate-validation/*`, `/admin/islamic-calendar` | `auth:super_admin_api`, `throttle:platform-sensitive`, Policies (issue #1917) | SA | Issue #1917 — `RateValidationAdminController` (pending/approve/reject) et `PublicHolidayController` passent par `TaxRatePolicy` / `PublicHolidayPolicy` : approbation/rejet des taux légaux et CRUD fériés réservés au SuperAdmin. Tests : `TaxRatePolicyTest`, `PublicHolidayPolicyTest`, `TaxSlabValidationWorkflowTest`, `PublicHolidayControllerTest`. |
...
| Payroll engine `/salary-*`, `/tax-slabs`, `/social-contributions`, `/payroll-runs`, `/bank-exports` | RW | R | - | RW | - | self payslips only | Self-service payslip routes are `/me/pay-slips*`; manager routes must not leak across tenant/FK chains. F-10 (#1540) : `GET /payroll-runs/{run}/journal` (journal de paie CSV, bulletins validés + totaux) — principal/comptable, isolation tenant. Tests : `PayrollJournalApiTest`. |
| Public holidays `/public-holidays` (tenant) | RW (entreprise uniquement) | - | - | - | - | - | Issue #1917 — routes réservées au manager `principal` (PublicHolidayPolicy) : CRUD limité aux fériés d'ENTREPRISE de sa société ; les fériés nationaux restent en lecture seule pour les tenants ; le SuperAdmin gère tout via `/api/v1/admin/public-holidays`. Tests : `PublicHolidayPolicyTest`, `PublicHolidayControllerTest`. |
...
| TaxSlab | TaxRatePolicy | managers + SA | managers + SA | managers + SA | managers + SA | managers + SA | SA only | Issue #1917 — `submit`/`history` aussi managers + SA ; l'isolation tenant (404 cross-company) reste au scope global `BelongsToCompany` + gardes contrôleurs (comportement inchangé) |
| SocialContribution | TaxRatePolicy | managers + SA | managers + SA | managers + SA | managers + SA | managers + SA | SA only | Idem TaxSlab (même policy) — approve/reject réservés au SuperAdmin (`/api/v1/admin/rate-validation/*`) |
| PublicHoliday | PublicHolidayPolicy | SA + P | — | SA + P | SA (tous) / P (fériés d'entreprise de sa société) | SA (tous) / P (fériés d'entreprise de sa société) | — | Issue #1917 — routes tenant `/api/v1/public-holidays` : principal uniquement ; fériés nationaux en lecture seule pour les tenants ; routes admin `/api/v1/admin/public-holidays` : SA |
