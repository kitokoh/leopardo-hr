...
| Payroll engine `/tax-slabs`, `/social-contributions` — workflow validation (#1813) | W (créer/soumettre) | - | - | W (créer/soumettre) | R/W (approuver/rejeter/historique) | - | ADMIN-PAIE #1813 : `PUT /tax-slabs/{id}/submit`, `PUT /social-contributions/{id}/submit` (manager principal/comptable) ; `PUT /platform/payroll/tax-rates/*/{id}/approve|reject` (super_admin uniquement) ; `GET .../history` audit trail immuable (`tax_rate_change_log`, append-only). Une ligne non-`active` est ignorée par le moteur de paie. Tests : `TaxSlabValidationWorkflowTest`, `PlatformTaxRateControllerTest`, `TaxRateChangeLogTest`. |
