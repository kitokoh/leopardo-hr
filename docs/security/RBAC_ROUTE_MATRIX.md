...
| Payroll engine `/salary-*`, `/tax-slabs`, `/social-contributions`, `/payroll-runs`, `/bank-exports` | RW | R | - | RW | - | self payslips only | Self-service payslip routes are `/me/pay-slips*`; manager routes must not leak across tenant/FK chains. F-10 (#1540) : `GET /payroll-runs/{run}/journal` (journal de paie CSV, bulletins validés + totaux) — principal/comptable, isolation tenant. Tests : `PayrollJournalApiTest`. #1823 : `GET /payroll-runs/{run}/declarations/cnps-cm` (CNPS Cameroun DAS CSV) — principal/comptable, 404 cross-tenant. Tests : `CnpsDeclarationTest`. |
