...
| FuelStation relevés `/api/v1/fuelstation/meters/{meter}/readings`, `/fuelstation/readings/{reading}/correct` (#5798) | `throttle:api`, `auth:sanctum`, `token.refresh`, `tenant`, `throttle:api-plan`, `api.manager:principal,rh` | P, RH | Relevés cumulés idempotents (delta, anomalie, rollover), corrections versionnées. Isolation tenant. Tests : `FuelMeterReadingTest`. |
