...
> **Date :** 2026-08-30 — **Périmètre audité :** code BC-25 livré (fondations, schéma, référentiel, POS/paiements, livraison, fidélité, promotions, rapports — lots 1xx..7xx, état au moment de l'audit).
...
| PII réservations | champs nécessaires au métier ; `notes_redacted` ; événements outbox **sans PII** (payload redigé) | ✅ |
| Vol de secret paiement | aucun secret en dur : identifiants passerelle en config/env, callback signé HMAC (secret par tenant) | ✅ |
| Rejeu de callback | signature HMAC + idempotence `UNIQUE(company_id, idempotency_key)` + montant vérifié serveur | ✅ |
| Double encaissement | montant recalculé serveur (`remainingDue`), transaction + re-vérification, 409 si déjà payé | ✅ |
| Caisse falsifiée | totaux recalculés serveur, écart + motif obligatoire, verrou optimiste `version` | ✅ |
| Points fidélité abusés | crédit idempotent (index unique partiel), échange jamais négatif (422), opt-in RGPD requis | ✅ |
| Promo hors bornes | validation serveur bornes/fenêtre/plafond, remise ≤ sous-total (BillCalculator) | ✅ |
| Export non autorisé | URL signée éphémère (15 min), signature invalide → 403, fichier résolu dans le dossier du tenant | ✅ |
| Rapports non autorisés | permission documentaire `restaurant.reports` (matrice RBAC RESTO-306) | ✅ |
| Dead-letter outbox silencieuse | statuts `failed` + `last_error` tracés ; alerting à configurer (R2) | ⚠️ |
| Logs PII | payloads outbox redigés (aucune PII client) — vérifié sur les événements publiés | ✅ |

## 3. PII clients — conformité RGPD

| Donnée | Localisation | Justification | Mesures |
|---|---|---|---|
| Nom/téléphone de réservation | `restaurant_reservations.contact_name/contact_phone` | nécessaire au service (spec §3.3) | données du tenant ; jamais dans les événements/rapports/CSV ; suppression avec le tenant |
| Lien client (fidélité) | `restaurant_loyalty_customers.customer_contact_id` | opt-in explicite requis | opt-in documenté (RESTO-606) ; solde exposé, contact jamais dupliqué |
| Notes | `notes_redacted` | contexte service | champ dédié « redacted » |
| Payloads de callback | `restaurant_order_payments.callback_payload_redacted` | traçabilité paiement | stocké redacté (JSONB) |

**Droit d'effacement** : couvert par la suppression du tenant (données `restaurant_*` tenant-scoped, aucune donnée globale hors tenant).
...
- `expected_cash_minor` = fonds + Σ encaissements confirmés (recalcul serveur) ; écart + motif obligatoire si écart.
- Événement de clôture `restaurant.pos.closed.v1` → Accounting/Reporting (RESTO-412).

## 6. Findings et recommandations

| # | Sévérité | Finding | Recommandation |
|---|---|---|---|
| R1 | Faible | Benchmarks de charge POS/réservations non exécutés | Gate `performance` du pilote avant GO |
| R2 | Faible | Alerting sur dead-letter outbox non branché | Alerter sur `restaurant_outbox_events.status=failed` |
| R3 | Faible | Rappel de réservation notifie l'équipe branche (pas encore le client par SMS) | Notification client via canaux externes BC-13 quand le CRM contact est branché (RESTO-80x) |
...
Surface enregistrée : `restaurant_pos` (voir `dev-hub/tools/security-threat-models.json`) — contrôles : secrets, signatures, replay, permissions, audit — référence : le présent audit + `THREAT_MODELS_MAT017.md`.
