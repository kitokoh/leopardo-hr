...
| IA conversationnelle | Assistance RH, recherche, recommandations, actions controlees | Prompts, contexte metier autorise, resultats, actions demandees | Utilisateurs autorises | Contrat, interet legitime, validation humaine selon action | **90 jours pour les logs d'audit IA** (`ai_audit_logs` **et `ai_tool_executions`** (#8164), purge planifiee `ai:purge-audit-logs`, config `ai.audit_log_retention_days` — #8144) ; pas de conservation des prompts au-dela | Tool calling permissionne, audit, masquage donnees sensibles, validation humaine, minimisation des payloads sortants (#8141), logs trial pseudonymises (#8144) |
