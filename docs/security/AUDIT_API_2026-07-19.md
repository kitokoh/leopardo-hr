...
> Périmètre : `api/` uniquement (Laravel 12, PHP ^8.2). Fait suite à `docs/audits/AUDIT.md` (2026-07-01/05) : ne recouvre pas les points déjà traités là-bas (Stripe/Chargily signature ✅ déjà corrigé, Redis Upstash déjà signalé, Mailables déjà créés).
...
| Sévérité | Constat |
|---|---|
| 🔴 Critique | `/api/v1/demo-users` public, en clair, en **production**, expose emails + mots de passe (valeur non citée, convention #1614) de comptes manager/admin réels sur l'instance déployée |
...
```
Retourne en clair :
- `admin@leopardo-rh.com` / `********` (rôle `super_admin`, accès `/platform`)
- 14 comptes manager/employé répartis sur 3 entreprises démo (TechCorp Algérie, PharmaPlus Casablanca, DigitalFlow Tunis), tous avec le mot de passe `********`
