...
Note 2026-09-02 (epic Attendance IA, lot 6 — ATT-004 #6769 / BIO-007 #6772 / AI-002 #6771) : nouvelle surface kiosque versionnee tenant-scoped —
- Enrolement biometrique kiosque : `POST /api/v1/kiosks/{deviceCode}/enrollments` (start : identifier + method face|fingerprint + template_payload + provider), `POST .../enrollments/{id}/activate` et `.../revoke` (manager_employee_id requis, validation manager active du tenant), `GET .../enrollments/status?identifier=` (etat neutre par methode, aucun gabarit). Middlewares `kiosk.device` (auth X-Kiosk-Token centralisee) et `kiosk.idempotency` (Idempotency-Key OBLIGATOIRE sur les ecritures d'enrolement : 422 MISSING/INVALID_IDEMPOTENCY_KEY, rejeu 24 h header `Idempotent-Replayed`). Reponses neutres (jamais de template/capture). Activation → flag employe `biometric_face_enabled`/`biometric_fingerprint_enabled` pose ; revocation → retire si plus aucun actif. Isolation : kiosque/employe/enrolement resolus dans le tenant (404 fail-closed).
- Offline kiosque borne et signe (BIO-007) : `POST .../sync` accepte une enveloppe optionnelle `device_state` {counter, nonce, signed_at, integrity HMAC-SHA256 (cle = sync_token en clair, message = device_code MAJ\ncounter\nnonce\nsigned_at ISO8601)} — falsification 422 `SYNC_INTEGRITY_MISMATCH`, rejeu 409 `SYNC_COUNTER_STALE`, horloge future 422 `SYNC_CLOCK_SKEW`, signature perimee 422 `SYNC_EXPIRED` ; evenement > fenetre `attendance.kiosk.offline.max_age_days` isole `EVENT_EXPIRED` ; `device_event_id`/`external_event_id` idempotents (aucune presence dupliquee) ; methode reellement utilisee preservee par evenement ; compteur acquitte `acked_event_counter` persiste. `GET /api/v1/kiosks/{deviceCode}/sync-status` (statut, compteur acquitte, heure serveur, politique offline) ; `GET .../config` et la reponse `/sync` exposent `offline_policy` + `server_time`. `POST .../punch` et `.../verify-face` acceptent `device_event_id` (rejeu idempotent).
- Punch badge/PIN/carte/manager sans flag biometrique employe (matrice BIO-006) ; roster expose `badge_number`.
- OCR compteurs FuelStation (AI-002) : `POST /api/v1/fuel-station/stations/{station}/pumps/{pump}/meters/{meter}/readings/ocr` (multipart photo ≤ 5 Mo, idempotency_key, → 202 row queued + job `ProcessMeterOcrJob`), `GET /api/v1/fuel-station/meter-ocr-requests/{ocr}` (statut/valeurs/confiance/anomalies), `POST .../{ocr}/review` (manager, accepted/rejected). Auto-comptabilisation uniquement confiance ≥ `METER_OCR_CONFIDENCE_THRESHOLD` sans anomalie ; jamais de clôture de session de pompe par l'OCR.
- Couvert par : `KioskEnrollmentApiTest`, `KioskOfflineSyncReliabilityTest`, `KioskSyncStatusEndpointTest`, `BiometricIsolationMatrixTest`, `KioskFingerprintRegressionTest`, `KioskOfflineSyncGuardTest` (unit), `MeterOcrFeatureTest`/`MeterOcrServiceTest` (FuelStation).
