...
Note 2026-06-01 : la liste mobile manager `GET /api/v1/employees` doit rester compatible avec `EmployeeResource` en production stricte. Les scenarios API doivent verifier que le payload pagine expose les champs utiles au mobile (`company_id`, contacts personnels, salaire, horaire, biometrie, `extra_data`, `work_state`) sans erreur de modele partiellement charge et sans fuite inter-tenant.
