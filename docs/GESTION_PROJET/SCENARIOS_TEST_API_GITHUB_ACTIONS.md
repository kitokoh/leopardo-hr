...
Note 2026-09-03 (lot audit fiabilité/sécurité batch 4, PR #6806, Closes #6529 #6535 #6536 #6548 #6554 #6557 #6559 #6560 #6563 #6754) : reprise de runs paie, verrous d'encaissements, claims bulk-pay, EdgeSync atomique.
- Paie batch (Closes #6529) : `POST /api/v1/payroll-runs/{run}/calculate` accepte désormais un run en `error` ou `processing` (orphelin worker) — scénarios : run `error` → 200 `calculated` ; run `processing` (worker mort) → 200 `calculated` ; run `validated` → 422 conservé. Retry `ProcessPayrollBatchJob` depuis `draft|error|processing` (claim conditionnel atomique). Tests : `ProcessPayrollBatchJobTest`, `PayrollRunControllerTest`.
- Encaisse comptable (Closes #6536) : `PaymentRegistrationService::register` et `DocumentWorkflowService::recordPayment` re-lisent le document sous `lockForUpdate()` dans la transaction — scénario : 2 encaissements avec instance stale → cumul exact, jamais de dépassement silencieux. Tests : `AccountingPaymentTest`.
- Bulk payment (Closes #6548) : un claim Redis sans artefact de paiement n'est plus compté réussi en silence — orphelin (> timeout job) volé et rejoué, claim récent signalé en `failures` (`completed_with_errors`). Tests : `ProcessBulkPaymentJobTest`.
- EdgeSync (Closes #6554) : `POST /api/v1/edge-node/{nodeId}/push` renvoie désormais `results[]` PAR ENREGISTREMENT (`queued`) — le daemon Edge (`EdgeDaemonSyncClient`) ne marque `synced` que les enregistrements acceptés ; un rejet reste `pending` (retry cycle suivant). Dédup : un re-push ne crée jamais 2 lignes `pending` (rafraîchit la ligne). Claim `pending→processing` conditionnel (`SyncEngineService::push`) — 2 syncs concurrents ne doublonnent plus. Tests : `EdgeSyncTest`, `EdgeDaemonSyncClientTest`.
- Idempotence (Closes #6557) : `Idempotency-Middleware` absorbe les pannes de cache en mode dégradé (requête traitée, jamais de 500 après commit) — scénario : store down → 200 + header d'absence de rejeu. Verrou anti-course 300 s. Tests : `IdempotencyMiddlewareTest`.
- Erreurs masquées (Closes #6559) : `error_message` des jobs (`GenerateBankExportJob`, `GeneratePaymentDocumentJob`) sanitized via `App\Support\SafeErrorMessage` (SQLSTATE/chemins/DSN → générique ; message métier conservé). `PayrollPaymentOrderService` nettoie le fichier banque sur rollback. Tests : `SafeErrorMessageTest`.
- Caméras (Closes #6560) : `GET /api/v1/view/cam` — le token ne voyage plus en query string (`?t=` supprimé, header `X-Token` seul) ; émission de stream token fail-closed hors local/testing sans `CAMERAS_STREAM_TOKEN_SECRET`. Tests : `CameraAccessTokensTest`, `CameraStreamTokenServiceTest`.
- Auth super-admin (Closes #6563) : login API verrouillé après 5 échecs / 15 min (compte, cache partagé) ; `PATCH /api/v1/platform/auth/profile` exige le mot de passe courant pour changer l'email (401 `INVALID_PASSWORD` / 422 si absent). Tests : `PlatformAuthTest`.
- Tests multi-fichiers (Closes #6754) : `RefreshTenantDatabase` re-migre à la frontière entre fichiers (registre `TenantRefreshRegistry`) — fin des erreurs en cascade 42P07/25P02 des suites locales séquentielles.
