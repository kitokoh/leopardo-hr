...
Note 2026-08-09 (F-08, #1538) : fin de contrat. `GET /api/v1/employees/{employee}/end-of-contract` (solde de tout compte : conges restants, indemnites, preavis, prorata) et `GET /api/v1/employees/{employee}/certificate-of-employment` (certificat de travail, mentions legales) — reserves RH/principal, PDF genere et archive dans le Cabinet. Scenarios : employee 403, cross-tenant 404, calculs golden, document archive + audit trail.
