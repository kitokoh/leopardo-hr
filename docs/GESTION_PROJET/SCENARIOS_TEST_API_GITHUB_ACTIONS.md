...
Note 2026-08-23 (issue #5234) : journal comptable — écritures débit/crédit dérivées des documents et paiements.
- Posting explicite et idempotent : `POST /accounting/documents/{document}/journal` (re-posting facture/avoir + paiements, updateOrCreate — jamais de doublon) ; seuls `invoice`/`credit_note` (statut ≠ draft/cancelled) et les paiements non-`pending` produisent des écritures (proforma/devis/BL/reçu sans impact comptable).
- Invariant légal : chaque posting et le journal complet restent équilibrés (Σ débit = Σ crédit) ; un posting déséquilibré lève `UnbalancedJournalEntryException` (rien n'est écrit).
- Plan de comptes PCF/SYSCOHADA simplifié : 411 Clients / 70 Ventes / 709 RRR / 4457 TVA / 512 Banques / 53 Caisse (selon méthode).
- `GET /accounting/journal?period=YYYY-MM` : écritures + totaux + `balanced`/`closed` ; RBAC comptable/principal (`api.manager:principal,comptable`) — employé → 403.
- `GET /accounting/journal/export.csv?period=YYYY-MM` : CSV UTF-8 BOM, séparateur `;`, colonnes `date;piece;libelle;compte;intitule;debit;credit`, ligne TOTAUX, cellules texte neutralisées anti-injection (#4169) — export exploitable par un expert-comptable.
- Clôture : `POST /accounting/journal/periods/{period}/close` (idempotent, 201) — période close = journal figé, tout posting daté dedans → 422 `PERIOD_CLOSED` (localisé ×4 via `errors.*`).
- Scénarios : facture équilibrée (3 écritures), avoir inversé, documents sans impact (0 écriture), paiement banque/cash (512 vs 53), pending non passé, re-posting idempotent + rafraîchit les montants, posting déséquilibré rejeté, journal par période (totaux + balanced), export CSV (BOM/en-tête/neutralisation/ligne totale), RBAC 403, clôture → re-posting 422, isolation tenant (B ne voit rien de A).
- Couverture : `AccountingJournalTest` (15 tests) — suite `tests/Feature/Accounting` 24/24.
