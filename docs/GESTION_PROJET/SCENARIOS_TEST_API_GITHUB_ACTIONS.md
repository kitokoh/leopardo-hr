...
Note 2026-08-09 (F-10, #1540) : `GET /payroll-runs/{run}/journal` exporte le journal de paie mensuel (CSV, toutes rubriques par employe) depuis un run verrouille/cloture. Les scenarios API doivent verifier : RBAC principal/comptable uniquement (employee 403), isolation tenant 404, run non cloture 422, totaux du CSV egaux aux totaux de la cloture, contenu horodate et rejouable.
Note 2026-08-10 (S-2, #1662) : les lectures de donnees sensibles sont journalisees dans `audit_logs` via `DataAccessAuditLogger` (metadata.category `hr_data_access`) : telechargement/consultation de bulletins (`hr_data.pay_slip_downloaded`/`pay_slip_viewed`), journal de paie (`hr_data.payroll_journal_exported`), export de run (`hr_data.payroll_run_exported`), export bancaire (`hr_data.bank_export_downloaded`), solde de tout compte (`hr_data.end_of_contract_viewed`), certificat (`hr_data.certificate_downloaded`), exports du dashboard (`hr_data.export`, metadata.export = nom de l'export). Echantillonnage configurable `audit.data_access.sample_rate` (defaut 1.0, deterministe par acteur+action). Scenarios a valider : `tests/Feature/Security/SensitiveDataAccessAuditTest` (un test par ressource sensible, taux 0 desactive, determinisme, commande `audit:sensitive-report`).
