...
Note 2026-08-14 (#1825) : Côte d'Ivoire pilot (ITSAS + CN + CNSS). Aucun endpoint HTTP ajouté/modifié — règles pays `CedeaoPayrollRules` (instance CI) passées de placeholder à pilot. Scénarios : ITSAS + CN cumulés (brut 3M → 655 500 XOF), CNSS plafonnée à 1 647 315 (retraite/famille) avec AT non plafonné, abattement 20 %, HS 3 paliers, 13ème mois obligatoire, les autres membres UEMOA inchangés (placeholder). Couverture : `GoldenCiItsasTest` + `AbstractCountryRulesCapTest`.
