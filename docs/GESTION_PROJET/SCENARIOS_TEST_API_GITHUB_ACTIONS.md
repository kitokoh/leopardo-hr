...
Note 2026-08-25 (issue #5444) : webhooks entrants — idempotence persistée. Scénarios à vérifier : (1) 1re livraison → effet + ligne `webhook_events` (source, event_id, response_code) ; (2) redelivrance même event_id → 200/201 rejoué SANS effet double (1 seul Payment / communication_event / lead) ; (3) livraison concurrente (response_code=0) → 202 ; (4) échec de traitement → 500 + réservation libérée (retry re-traite) ; (5) signature invalide → 400 sans effet ni ligne ; (6) sources sans id (bounce/lead) → clé = sha256(payload) ; (7) isolation tenant : payload forgé non signé n'atteint jamais la résolution.
