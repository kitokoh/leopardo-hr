...
## Issue #1662 — Journalisation des accès aux données sensibles (Spec S-2)

Les lectures sensibles sont tracées dans `audit_logs` via `DataAccessAuditLogger::recordSensitive()` (catégorie `sensitive_data_access`, échantillonnage configurable `audit.sensitive_access.sampling`) : `GET /pay-slips/{id}/pdf` et `/me/pay-slips/{id}/pdf` (action `hr_data.pay_slip_downloaded`), `/me/payment-documents/{id}/download` (`hr_data.payment_doc_downloaded`), `/payroll-runs/{run}/journal` (`hr_data.payroll_journal_viewed`), `/bank-exports/{id}` et `/bank-exports/{id}/download` (`hr_data.bank_export_viewed` / `hr_data.bank_export_downloaded`), `/employees/{id}/end-of-contract` (`hr_data.end_of_contract_viewed`), `/employees/{id}/certificate-of-employment` (`hr_data.certificate_downloaded`). Nouvelle commande console `audit:sensitive-report` (`--days`, `--company`). Aucun endpoint HTTP ajouté/modifié (mêmes routes, mêmes RBAC).

- Scenario a valider : `tests/Feature/SensitiveAccessAuditTest` (un test par ressource sensible, échantillonnage 0.0 → aucune trace, rapport commande) + volume borné par la config de sampling.
