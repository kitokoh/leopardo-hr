...
Note 2026-08-14 (#1811/#1812) : jours fériés par pays + calendrier islamique dynamique.
- `GET|POST /admin/public-holidays`, `PUT|DELETE /admin/public-holidays/{id}` (super-admin, nationaux) ; `GET|POST /public-holidays`, `PUT|DELETE /public-holidays/{id}` (manager principal, entreprise) ; `GET /admin/islamic-calendar`, `PUT /admin/islamic-calendar/{holidayKey}/{year}`, `POST /admin/islamic-calendar/confirm-year/{year}` (super-admin). Scénarios : CRUD national/entreprise, RBAC (403/401), fusion fériés fixes+islamiques dans `workingDaysBetween`, confirmation de date → `confirmed=true`, durée 2 jours (CM) → 2 jours chômés. Couverture : `PublicHolidayControllerTest`, `PublicHolidayServiceTest`, `IslamicCalendarControllerTest`, `IslamicCalendarServiceTest`.
