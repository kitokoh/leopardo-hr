...
## Issue #1663 — Durcissement paie (Spec S-3)

Trois durcissements : (a) `social_contributions.effective_from` passe NOT NULL via migration additive avec backfill (garde `information_schema`, inoffensive sur env neufs) ; la validation store `effective_to` après `effective_from` est couverte par un test API (422 attendu). (b) `PayrollCycleService::safeEmployeeBalance` propage l'erreur (500 explicite + `Log::error`) au lieu du fallback silencieux — `GET /payroll/mobile-summary` remonte désormais les échecs de calcul de paie. (c) Migrations additives de rattrapage pour geo_auto (`attendance_logs.method`) et company_id uuid (`zkteco_devices`, `kiosk_announcements`), avec commentaires de renvoi dans `2026_06_29_000205` et `2026_05_18_000003`. Aucun endpoint HTTP ajouté/modifié.

- Scenario a valider : `tests/Feature/Payroll/PayrollHardeningTest` (NOT NULL, geo_auto accepté sur base fraîche, uuid kiosque, validation store, propagation d'erreur mobile-summary) + `migrate:fresh` + `RefreshTenantDatabase` verts.
