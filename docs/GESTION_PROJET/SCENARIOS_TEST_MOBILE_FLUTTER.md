...
Note 2026-09-13 (PR #7333, Refs #7329) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,tr,ar}.arb` uniquement parce que les fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : l'ajout de la clé partagée `navigation.clientModules` (titre de la section « Modules des entreprises clientes » du menu de l'admin plateforme) est propagé mécaniquement à tous les targets du catalogue, ARB compris. Cette clé n'est consommée par aucun écran mobile — la navigation mobile est inchangée. Surface de tests mobile inchangée (suites widget/unit existantes par app — voir `front/mobile_apps/<app>/test/`).

Note 2026-09-14 (éditabilité des e-mails, #7347) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,ar,tr,en}.arb` uniquement parce que la clôture i18n du dépôt exige de faire tourner **les 3 synchronisations** (`sync-web`, `sync-backend`, `sync-mobile`) dès qu'une clé est ajoutée à `shared/i18n/locales/*` — sinon la CI `i18n-enterprise.yml` échoue en détectant des fichiers générés non committés. Les clés ajoutées (`navigation.emailTemplates`, `emailsAdmin.*`) servent l'écran **web** d'administration des e-mails ; les ARB mobiles les reçoivent par construction (catalogue partagé), sans nouvel écran ni nouveau test côté Flutter. Vérifié : `node shared/i18n/sync/sync-mobile.js` → `I18N_SYNC_MOBILE_OK`, parité ×4 (`check-i18n-catalog-parity.sh`) **verte**.
Note 2026-09-14 (PR #7350, Refs #7351) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,tr,ar}.arb` uniquement parce que ces fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : trois clés partagées de l'écran « Mon compte » du client web (`settingsPage.company`, `settingsPage.manage2fa`, `settingsPage.tenantSubtitle`) sont propagées mécaniquement à tous les targets du catalogue, ARB compris. Aucune de ces trois clés n'est consommée par un écran mobile — la navigation et les écrans mobiles sont inchangés. Surface de tests mobile inchangée (suites widget/unit existantes par app — voir `front/mobile_apps/<app>/test/`).
>

Note 2026-09-15 (PR #7425, BC-19 Caméras) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,tr,ar}.arb` uniquement parce que ces fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : le nouveau bloc partagé `cameras.*` (mur de caméras, détail, permissions, jetons d'accès tiers, viewer public) sert la **surface web client** ; les ARB mobiles le reçoivent par construction (catalogue partagé), sans nouvel écran ni nouveau test côté Flutter — l'app mobile caméras reste #7426. Vérifié : `node shared/i18n/sync/sync-mobile.js` → `I18N_SYNC_MOBILE_OK`, `node shared/i18n/validators/validate.js` → `I18N_VALIDATION_OK`, parité ×4 (`check-i18n-catalog-parity.sh`) **verte**. Même situation que la note du 2026-09-14 (#7347, PR #7350).


Note 2026-09-16 (PR #7465, formation horizontale + plancher RH solo, #7432/#7423) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,ar,tr}.arb` uniquement parce que ces fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : les 8 clés de la fiche entreprise côté **web/admin** (`companyDetail.features.{rh,finance,ai,cameras,tracking,planning,training,cabinet`) sont propagées mécaniquement à tous les targets du catalogue, ARB compris. Vérifié : **aucune** de ces clés n'est consommée par un écran mobile (`grep -rl companyDetail front/mobile_apps/*/lib` → 0 occurrence), la navigation et les écrans mobiles sont inchangés. Surface de tests mobile inchangée (suites widget/unit existantes par app — voir `front/mobile_apps/<app>/test/`). Vérifié : `node shared/i18n/sync/sync-mobile.js` → `I18N_SYNC_MOBILE_OK`, parité ×4 (`check-i18n-catalog-parity.sh`) **verte**.
>
Note 2026-09-19 (PR #7714, issue #7708 — positionnement « suite métier », étape 2 vitrine & SEO) : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,tr,ar}.arb` uniquement parce que ces fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : le débrandage des clés partagées (`seo.llms.summary` canonique ×4, `systemPage`, `welcome`) est propagé mécaniquement à tous les targets du catalogue. Aucun écran mobile ne change ; les `android:label` / bundle ids restent inchangés (identifiants §4 du référentiel P03). Surface de tests mobile inchangée. Vérifié : `I18N_SYNC_MOBILE_OK` + `I18N_VALIDATION_OK (4 locales)`.

Note 2026-09-16 (PR #7518, issue #7428 — positionnement « suite métier ») : **aucun changement de comportement des apps mobiles.** Le diff touche `front/mobile_apps/leopardo_core/lib/l10n/app_{fr,en,tr,ar}.arb` uniquement parce que ces fichiers sont **générés** par `shared/i18n/sync/sync-mobile.js` : le changement de `app.title` (nom produit « Leopardo » + catégorie « suite métier » au lieu de « Leopardo RH ») et le nouveau namespace **`seoRoot`** (phrases canoniques partagées avec la vitrine et le manifeste PWA) sont propagés mécaniquement à tous les targets du catalogue. Aucun écran mobile ne consomme `seoRoot` ; le libellé affiché sous l'icône d'une app reste son **nom d'application** (`android:label` / `CFBundleDisplayName` — « Leopardo RH » pour l'app RH & paie, « Leopardo Employee », « Leopardo Manager », « Leopardo Platform Admin »), conformément à la décision : « Leopardo RH » est une **application de la suite**, pas la catégorie du produit. Les bundle ids sont **inchangés**. Surface de tests mobile inchangée (suites widget/unit existantes par app). Vérifié : `I18N_SYNC_MOBILE_OK` + `I18N_VALIDATION_OK (4 locales)`.
