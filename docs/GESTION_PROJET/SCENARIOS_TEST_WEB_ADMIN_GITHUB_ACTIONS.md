...
## Note de conservation — échappement XML de l'avatar à initiales (issue #8022 tranche 4, 2026-09-21)

**Aucun changement de comportement visible de la console admin.** Le diff ne touche que `localInitialsAvatar` (`src/views/users/UsersView.vue`) : les initiales interpolées dans le SVG data-URI sont désormais échappées XML (`& < > " '`) — un nom contenant `<` ou `&` cassait le XML après décodage de l'URI (avatar vide, voire markup injecté). Pour tous les noms usuels (lettres, accents, espaces), l'avatar rendu est **strictement identique** : les scénarios existants restent valides tels quels. Couverture manuelle recommandée lors d'un prochain passage e2e : renommer un utilisateur « S & P » → l'avatar affiche « S & » correctement.
