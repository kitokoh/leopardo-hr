...
| Edge Nodes offline | docs/GESTION_PROJET/SCENARIOS_TEST_API_GITHUB_ACTIONS.md | Tests - Leopardo RH | EdgeControllerTest | backend-tests |

- v4.18.0 : Edge Nodes offline — Infrastructure complète déploiement nœuds Edge. Nouveaux endpoints publics : `GET /edge/install.sh`, `GET /edge/download/docker-compose.yml`, `GET /edge/license-public-key`, `POST /edge/heartbeat`. Endpoints platform super-admin : `GET/POST/DELETE /platform/edge/nodes`. Cron `edge:detect-silent-nodes` (everyFiveMinutes, withoutOverlapping). Migration table `edge_nodes`. Image Docker `leopardo/edge-api:1.0.0`. PWA web offline `front/web-offline/`. Dashboard Vue `EdgeNodesView`. ZKTeco config edge mode. Drift SQLite mobile (`EdgeDatabase`). Scenarios ajoutes dans SCENARIOS_TEST_API_GITHUB_ACTIONS.md.
