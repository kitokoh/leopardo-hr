...
- 2026-09-03 (PR #6806, lot audit fiabilité/sécurité batch 4 — Closes #6529 #6535 #6536 #6548 #6554 #6557 #6559 #6560 #6563 #6754) : recalcul paie depuis error/processing + claim conditionnel batch job ; verrous `lockForUpdate` encaissements ; re-vérification des claims bulk-pay (orphelin rejoué, jamais « payé à tort ») ; EdgeSync conditionnel + réponse push par enregistrement + dédup ; idempotence dégradée cache ; `SafeErrorMessage` (erreurs masquées) ; secret stream token caméra fail-closed + viewer X-Token only ; lockout login super-admin + email require password ; re-migration tenant entre fichiers de test. Scénarios ajoutés dans SCENARIOS_TEST_API_GITHUB_ACTIONS.md.
