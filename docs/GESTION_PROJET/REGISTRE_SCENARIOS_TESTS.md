...
- v4.16.128 : `AuthServiceTest::test_login_resolves_public_company_when_tenant_schema_shadows_companies_table` couvre le login shared PostgreSQL quand le schema tenant masque `public.companies`. Tout smoke demo doit verifier `POST /auth/login` puis `GET /auth/me`, pas seulement `/demo-users`.
