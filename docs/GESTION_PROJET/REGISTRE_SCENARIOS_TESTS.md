...
## Extension 2026-08-24 — Onboarding multi-profils et recrutement candidat

| Domaine | Scénarios nominaux | Refus / résilience | Contrôles CI et artefacts |
|---|---|---|---|
| Onboarding personnel pivot | Sélection cumulative des statuts étudiant, employé, entrepreneur et recherche d’emploi ; modification ultérieure depuis les paramètres ; conservation des CV et documents | Statut recherche d’emploi requis pour les recommandations ; pointage désactivé tant qu’une demande de rattachement n’est pas acceptée ; validation d’une sélection vide | Tests API d’onboarding, couverture Backend/Coverage, contrôles i18n fr/en/ar et validation mobile/web |
| Recommandations et candidatures | Profil de recherche, recommandations hybrides règles + IA, plusieurs CV, candidature authentifiée et suivi présélection/entretien/offre | CV invalide ou absent, profil non actif, application sans rattachement ou notification de changement d’étape | Tests Recruitment/User, OpenAPI, i18n, Flutter/Web build et rapports JUnit/Coverage |

Le contrat fonctionnel est porté par `JobRecommendationService`, les contrôleurs d’authentification et de candidature, ainsi que les écrans `front/mobile_apps/leopardo_employee` et `front/web`. Toute évolution doit conserver la compatibilité fr/en/ar et le verrou métier empêchant le pointage avant acceptation par l’entreprise.

## Extension 2026-08-24 — Contrat CI de la PR #5296

La PR #5296 (`Closes #5261`) ajoute le scénario d’activation progressive des personas sans suppression du compte personnel. La validation de fusion exige les contrôles Backend, Coverage, PHPStan, Architecture, OpenAPI, I18N, Mobile split guard, actionlint, sécurité et Web/Vercel lorsque les surfaces correspondantes sont modifiées.
