...
- CRM V0 (#5705-#5716, essaim 2026-08-28) : nouveau module CRM client tenant — migrations `crm_pipelines`/`crm_pipeline_stages`/`crm_leads`/`crm_opportunities`/`crm_activities`/`crm_tasks`/`crm_task_assignees`, surface API `/api/v1/crm/*` (leads, opportunities, pipelines, stages, tasks, activities) derriere `api.manager:principal,rh`, Policies dediees `app/Policies/Crm*`, validation stricte (champs inconnus 422 `unknown_fields`, enums/tris allowlistes), isolation tenant (404 cross-tenant), timeline append-only, partage explicite des taches. Scenarios couverts par `CrmPoliciesTest`, `CrmApiTest`, `CrmActivitiesTasksTest`, `CrmMigrationsInvariantsTest`, `CrmModelsTenantScopingTest`, `OpenApiCrmContractTest`.
| CRM V0 client | docs/GESTION_PROJET/SCENARIOS_TEST_API_GITHUB_ACTIONS.md | Tests - Leopardo RH | CrmPoliciesTest, CrmApiTest | backend-tests |
