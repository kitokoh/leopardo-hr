...
## Complément audit #6606 (2026-09-01) — vars et secrets manquants

### Vars manquantes (13 au total, complément de la table ci-dessus)

| Variable | Used by | Purpose | Required? |
|---|---|---|---|
| `PROD_API_BASE_URL` | `e2e-staging.yml` | URL API de prod pour les E2E Playwright — **fail-closed** : le workflow échoue si absente (`e2e-staging.yml:57-60`) | Required for E2E |
| `PROD_ADMIN_URL` | `e2e-staging.yml` | URL admin de prod pour les E2E Playwright (fail-closed idem) | Required for E2E |
| `PROD_WEB_URL` | `e2e-staging.yml` | URL vitrine de prod pour les E2E Playwright (fail-closed idem) | Required for E2E |
| `E2E_MAILCATCHER_URL` | `e2e-staging.yml` | URL Mailcatcher pour intercepter les emails des E2E | Required for E2E |
| `NEXT_PUBLIC_SITE_URL` | `deploy-admin-dashboard.yml` | URL publique du site (injectée côté admin-dashboard) | Optional |
| `MERGE_DAILY_QUOTA` | `merge-quota-guard.yml` | Quota quotidien de merges autorisés par le guard | Optional (default in workflow) |
| `QUEUE_MAX_PENDING` | `queue-supervision.yml` | Seuil d'alerte jobs pending en file prod | Optional (default in workflow) |
| `QUEUE_MAX_FAILED` | `queue-supervision.yml` | Seuil d'alerte jobs failed en file prod | Optional (default in workflow) |
| `QUEUE_MAX_STALE_MINUTES` | `queue-supervision.yml` | Âge max d'un job en file avant alerte (minutes) | Optional (default in workflow) |
| `FIX_FEAT_RATIO_MAX` | `fix-feat-ratio-guard.yml` | Ratio fix/feat max avant blocage | Optional (default in workflow) |
| `FIX_FEAT_RATIO_WARN` | `fix-feat-ratio-guard.yml` | Ratio fix/feat déclenchant un warning | Optional (default in workflow) |
| `FIX_FEAT_RATIO_DAYS` | `fix-feat-ratio-guard.yml` | Fenêtre (jours) du ratio fix/feat | Optional (default in workflow) |
| `FIX_FEAT_RATIO_ENFORCE` | `fix-feat-ratio-guard.yml` | `"true"` = le guard bloque en échec (sinon warning only) | Optional (default in workflow) |

### Secrets queue/workflows (8, jamais documentés jusqu'ici)

| Secret | Used by | Purpose | Required? |
|---|---|---|---|
| `APP_KEY` | `queue-supervision.yml`, `queue-worker-fallback.yml` | Clé Laravel APP_KEY pour les jobs queue supervisés | Required for queue workflows |
| `APP_URL` | `queue-supervision.yml`, `queue-worker-fallback.yml` | URL de l'app pour les jobs queue | Required for queue workflows |
| `DB_HOST` / `DB_PORT` | `queue-supervision.yml`, `queue-worker-fallback.yml` | Hôte/port Postgres des queues | Required for queue workflows |
| `DB_DATABASE` / `DB_USERNAME` / `DB_PASSWORD` | `queue-supervision.yml`, `queue-worker-fallback.yml` | Identifiants Postgres des queues | Required for queue workflows |
| `SLACK_MONITORING_WEBHOOK_URL` | `queue-supervision.yml`, `queue-worker-fallback.yml`, `ci-observability.yml` | Webhook Slack des alertes de supervision | Optional (alertes seulement) |

> **Rappel** : `DB_*` (sans suffixe) des workflows queue ≠ `DATABASE_URL` des workflows tests — deux jeux de secrets distincts.
