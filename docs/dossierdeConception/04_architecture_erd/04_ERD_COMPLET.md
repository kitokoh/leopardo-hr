...
manager_id          INT         FK → employees.id NULL  -- Responsable hiérarchique (UNIFIÉ SQL/ERD — suppression supervisor_id)
zkteco_id           VARCHAR(50)  NULL                   -- ID lecteur biométrique ZKTeco
...
status              VARCHAR(20)  NN DEF 'active'  CHECK IN ('active','suspended','archived')
last_login_at       TIMESTAMPTZ NULL
created_at          TIMESTAMPTZ NN DEF NOW()
updated_at          TIMESTAMPTZ NN DEF NOW()

INDEX: (email), (matricule), (department_id), (role), (status)
```
...
amount_remaining    DECIMAL(12,2) NN DEF 0  -- Solde restant, mis à jour par PayrollService chaque mois
