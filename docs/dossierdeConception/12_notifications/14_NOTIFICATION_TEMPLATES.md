...
## STRATÉGIE NOTIFICATIONS TEMPS RÉEL — WEB DASHBOARD (Vue.js / Inertia.js)

### Décision d'architecture : Server-Sent Events (SSE)

**Choix retenu : SSE (Server-Sent Events)**
- ✅ Compatible o2switch (Nginx + PHP-FPM) — pas besoin de WebSocket server
- ✅ Unidirectionnel serveur → client (suffisant pour les notifs)
- ✅ Reconnexion automatique native
- ❌ WebSockets : requiert un serveur séparé (Soketi/Pusher) — surcoût infra MVP
- ❌ Polling : gaspille des requêtes, mauvaise UX

### Endpoint SSE Laravel

```php
// Route
GET /api/v1/notifications/stream   (Auth:sanctum)
...
public function stream(Request $request): StreamedResponse
{
    return response()->stream(function () use ($request) {
        ...
        while (true) {
            // Récupérer les nouvelles notifs depuis Redis ou DB
            $notifs = Notification::where('employee_id', $employee->id)
            ...
            // Heartbeat toutes les 30s (évite timeout Nginx)
            echo ": heartbeat\n\n";
            ob_flush();
            flush();
        ...
        'X-Accel-Buffering' => 'no',  // CRITIQUE pour Nginx — désactive le buffering
        'Connection'        => 'keep-alive',
    ]);
}
```

### Config Nginx requise (ajouter dans nginx-api.conf)

```nginx
location /api/v1/notifications/stream {
    proxy_pass         http://127.0.0.1:9000;
    proxy_buffering    off;           # CRITIQUE — désactive le buffering SSE
    proxy_cache        off;
    proxy_read_timeout 3600s;         # 1h max de connexion SSE
    chunked_transfer_encoding on;
}
```

### Implémentation Vue.js (Composable)
...
export function useNotifications() {
    ...
    const connect = (token) => {
        ...
        eventSource.onerror = () => {
            // Reconnexion automatique native SSE après 3s
            console.warn('SSE disconnected, reconnecting...')
        }
    }

    const disconnect = () => eventSource?.close()

    onMounted(() => connect())
    onUnmounted(() => disconnect())

    return { notifications, unreadCount, disconnect }
}
```

### Compteur Redis (performance)

```php
// À chaque nouvelle notification créée :
Redis::incr("tenant:{$companyUuid}:notif:{$employeeId}:unread");

// À la lecture (GET /notifications/mark-read) :
Redis::set("tenant:{$companyUuid}:notif:{$employeeId}:unread", 0);

// Le SSE lit depuis Redis en priorité avant DB pour la perf
```
