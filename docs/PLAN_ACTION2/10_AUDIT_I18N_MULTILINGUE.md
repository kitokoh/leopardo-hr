...
- ~~`dev-hub/tools/validate-i18n-debt.ps1` produit un rapport utile comme point de depart mais peu fiable pour piloter...`~~ **Resolu (PA2-I18N-015)** : remplace par `dev-hub/tools/i18n-debt.js` (Node, coherent avec le reste de l'outillage i18n), qui ignore les classes CSS/Tailwind, les listes de declarations CSS inline, les tokens techniques (routes, identifiants, expressions de template/JS liees) et separe le texte dev/log (console/print/debugPrint/TODO) du texte reellement visible par l'utilisateur. Rapport republie `docs/validation/I18N_DEBT_REPORT_2026_07_22.md` : 7604 signaux utilisateur reels (contre 11 642 signaux bruites precedemment), signal desormais exploitable pour piloter la migration.
- ~~Aucun garde CI ne bloque aujourd'hui l'introduction d'une **nouvelle** chaine en dur sur une PR...~~ **Resolu (PA2-I18N-014)** : nouveau job bloquant `check-new-hardcoded-strings` dans `.github/workflows/i18n-enterprise.yml`, execute `dev-hub/tools/check-i18n-diff.js` sur les lignes ajoutees du diff de PR (mobile employee/manager/platform_admin, kiosk, admin-dashboard, web app/modules, PDF/emails API) ; ne bloque que les *nouvelles* chaines, la dette preexistante (mesuree par `dev-hub/tools/i18n-debt.js`, PA2-I18N-015) reste hors perimetre de ce garde.
