...
## Suivi PA2-API-003 — couverture automatisee (2026-07-25)

`PA2-API-003` (Issue #990) demande que chaque bouton critique ait une route/endpoint **ou un test**, pas seulement un audit manuel ponctuel. En comparant cette matrice a `api/tests/Feature/FrontendApiContractTest.php` (le test de contrat automatise qui pin chaque route critique cote mobile/kiosk/platform), 6 routes deja auditees manuellement ci-dessus n'avaient encore aucune assertion automatisee : le calcul/validation de run de paie et la liste des bulletins (admin web, section 5), le telechargement de bulletin PDF cote admin web (deja couvert cote mobile mais pas cote admin), et l'approbation/rejet de conge (admin web, section 5). Ajoutees a `criticalFrontendRoutes()` : `admin payroll run calculate`, `admin payroll run validate`, `admin pay slips list`, `admin pay slip pdf`, `admin absence approve`, `admin absence reject`. `php artisan test --filter=FrontendApiContractTest` : 121/121 vert (114 preexistants + 7 nouveaux jeux de donnees — l'ecart entre 6 routes ajoutees et 7 datasets vient de la deduplication naturelle du data provider, `admin pay slip pdf` et `mobile pay slip pdf` restant deux entrees distinctes cote mobile/admin comme le reste de la matrice). Les boutons a action locale (kiosk demo, pagination web dashboard, sandbox checkout) restent intentionnellement hors de ce test de contrat (pas d'appel reseau a verifier), conformement a leur justification dans la matrice ci-dessus. Le formulaire signup et le checkout vitrine (section 7) restent hors perimetre de ce test Laravel : ce sont des routes internes Next.js (`front/web/src/app/api/forms/signup/route.ts`, `front/web/src/app/api/billing/checkout/route.ts`) qui proxifient vers le backend, pas des routes Laravel directement testables par `Route::getRoutes()`.
