...
| PA2-MKT-006 | P1 | Fait le 2026-07-25: `SocialProofMetrics` affichait "500+ entreprises actives"/"50K+ employes geres"/"99.9% SLA" alors que `PILOTAGE.md` confirme 0 client payant et qu'aucun monitoring uptime n'est configure (chiffres fabriques) -> remplaces par 4 faits verifiables dans le depot (pays de paie dedies, langues produit, surfaces produit, tests backend automatises); `TrustedBrands` (secteurs adresses, PA2-MKT-011) et la video demo (`ProductDemoVideo`, PA2-MKT-014) etaient deja livres avant ce ticket | `front/web` | FAIT: aucun chiffre non verifiable affiche; section "trusted by" deja requalifiee en secteurs (PA2-MKT-011); video/assets existants deja integres (PA2-MKT-014); `tsc`/`eslint`/tests vitrine (239 tests) verifies verts |
...
| PA2-MKT-010 (lot Marketeur, issue #1280) | P0 | Fait le 2026-07-26 (audit): Backend & Base de donnees module Marketeur (migrations `social_accounts`/`social_posts`, `AyrshareClient`, `SocialPostController`). **Note**: cet ID collisionne avec l'entree PA2-MKT-010 ci-dessus (avatars vitrine, autre lot) ; voir `27_AUDIT_STATUT_PA2_MKT_010.md` pour la clarification | `api/app/Modules/Marketing/**`, `api/openapi.yaml` | FAIT: 3 criteres d'acceptation de l'issue #1280 deja satisfaits par du code existant et teste (Phases 1-3 anterieures) ; seul ecart reel trouve et corrige: `api/openapi.yaml` n'avait aucune couverture des 6 routes `/marketing/*` (seul module sans contrat OpenAPI) -> tag `Marketing` + 6 chemins documentes, `npx @redocly/cli lint` valide sans erreur |
...
| PA2-MKT-011 (lot Marketeur, issue #1281) | P0 | Fait le 2026-07-26: Dashboard Web Marketing (Lot 2) — layout `(marketing)`, page calendrier `/social`, composant `PostEditor`. **Note**: cet ID collisionne avec l'entree PA2-MKT-011 ci-dessus (TrustedBrands vitrine, autre lot) | `front/web/src/app/(dashboard)/(marketing)/**`, `front/web/src/modules/marketing/**` | FAIT: nouveau groupe de routes `(marketing)` avec sous-navigation Calendrier/Liste, page `/social` (calendrier mensuel, creation de post par jour, apercu/publication/suppression), composant `PostEditor` reutilisable (aussi branche sur l'existant `/social-marketing`) ; `tsc`/`eslint`/`next build`/tests unitaires (11 nouveaux) et e2e (1 nouveau, 7/7 verts) verifies |
...
| PA2-MKT-014 | P1 | Fait le 2026-07-25: nouvelle section `ProductDemoVideo` integree sur l'accueil et `/demo`, video reelle (63.96s) issue des captures existantes `assets/videos/{landing,admin,mobile}_demo.webm` (jamais utilisees hors README), sous-titres fr/en, poster reel | `front/web/src/modules/vitrine` | FAIT: video 60-120s reelle integree sur l'accueil et `/demo`, hebergee via `public/videos` (mp4+webm), sous-titree fr/en (WebVTT); `tsc`/`eslint`/tests vitrine (239 tests)/`next build` verifies verts |

## Onboarding client et trial

| ID | Priorite | Ticket | Surface | Definition of Done |
|---|---|---|---|---|
| PA2-ONB-001 | P0 | ~~Trial self-service de bout en bout~~ Fait (audit 2026-07-25, `25_AUDIT_STATUT_PA2_ONB_001_A_003.md`): `SelfServiceTrialController` couvre deja signup OTP, verification, provisioning tenant+manager, email welcome credentials, next-steps et drip J+1/J+3/J+7, teste par `SelfServiceTrialTest.php` (5 tests) | API, `front/web` | FAIT: creation entreprise, manager, plan trial, email welcome, retour credentials ou next-step securise |
| PA2-ONB-002 | P0 | ~~Activation client platform admin~~ Fait (audit 2026-07-25, `25_AUDIT_STATUT_PA2_ONB_001_A_003.md`): creer/voir/activer deja livres (`CompanyProvisioningService`, `CompanyDetailView.vue` `activateClient()`), pays/devise/langue/timezone via `CountryDefaults`, teste par `PlatformCompanyProvisioningTest.php` | API, `front/admin-dashboard`, mobile admin | FAIT: creer, voir, activer client; devise/langue/timezone selon pays; tests contrat |
| PA2-ONB-003 | P1 | ~~Onboarding wizard manager~~ Fait (audit 2026-07-25, `25_AUDIT_STATUT_PA2_ONB_001_A_003.md`): `OnboardingWizard.tsx` (web) et `onboarding_screen.dart` (mobile manager) couvrent horaires/employes/branding/regles/kiosk via un checklist API-backed (`App\Modules\Onboarding`), largement teste (6 fichiers de tests API + 1 mobile) | `front/web`, mobile manager | FAIT: premiere connexion guide horaires, employes, branding, regles, kiosk |
...
| PA2-MOB-003 | P0 | ~~Pointage employee multi-evenements~~ Fait (audit 2026-07-25, `26_AUDIT_STATUT_PA2_MOB_003.md`): tous les work_type (normal/break/resume/mission/travel/overtime/training/other) et la vue details jour (`/attendance/today`, summary) sont deja livres et testes (`MultiPunchTest.php`), parite kiosk confirmee (PA2-ATT-010) | API, employee mobile | FAIT: arrivee simple, pause, reprise, mission, depart, heure supp; details jour listent tout |
...
| PA2-MOB-010 | P2 | ~~Design system mobile 2026~~ Fait (audit 2026-07-25, `24_AUDIT_STATUT_PA2_MOB_010.md`): les 3 ecarts concrets releves par `12_AUDIT_MOBILE_DESIGN_UX.md` (litteraux hex dupliques, dark mode force sans decision documentee, `leopardo_platform_admin` sans vocabulaire de composants partages) sont chacun deja fermes par `PA2-MOB-011`/`012`/`013`, re-verifies par mesure directe (0 litteral hex restant, decision ecrite, 58 usages de composants partages dans `leopardo_platform_admin`) | core + apps | FAIT: composants unifies, contrastes lisibles (WCAG AA verifie), dark mode coherent et documente |
...
| PA2-QA-007 | P1 | Fait le 2026-07-25: CORS/TrustProxies deja corriges et testes (`docs/security/AUDIT_API_2026-07-19.md`, `CorsAndTrustedProxyTest`); gap reel trouve et corrige sur le cold-start admin-dashboard (`services/api.js` ne retentait jamais les 502/503/504, contrairement a la vitrine et au mobile) | API/web/mobile | FAIT: web vitrine et apps gerent Render cold-start et CORS proprement |
...
| PA2-OPS-008 | P1 | ~~Forcer~~ Fait: les pratiques GitHub Issues (PR Template, Labels) | `.github` | FAIT (issue #1279): `.github/PULL_REQUEST_TEMPLATE.md` rappelle explicitement que toute PR doit inclure `Closes #XXX` ; `.github/ISSUE_TEMPLATE/feature.yml` ajoute les champs obligatoires `Surface (ex: api, front/web)` et `Criteres d'acceptation` ; nouveau garde CI **bloquant** `dev-hub/tools/check-pr-closes-issue.sh` (branche dans `plan-action2-claim-guard.yml`) qui refuse toute PR sans reference `Closes`/`Fixes`/`Resolves #<n>`, sauf typage `docs:`/`chore:` (meme exemption que PA2-AUTO-004) |
