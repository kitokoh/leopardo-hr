...
- **PHPStan** — `phpstan.neon` declare `level: max` pour l'ensemble de `app/`. État CI réel (bloquant) :
  - **`phpstan.neon` (level max) — diff-gate sur fichiers modifiés** : le job `Backend Quality` de `tests.yml` exécute `phpstan.neon` (avec `phpstan-baseline.neon`, dette gelée ~historique) sur **tous les fichiers backend modifiés du PR** (`app/**` + `routes/**`, issue #5590). Toute NOUVELLE erreur level max sur un fichier modifié fait échouer le PR ; les erreurs pré-existantes sont ignorées via le baseline.
  - `phpstan-modules.neon` (niveau 5, `app/Core`/`app/Modules`/`app/Shared`) — job `phpstan-modules` (`architecture-check.yml`), périmètre complet.
  - `phpstan-strict.neon` (niveau 8, meme perimetre) — job `phpstan-strict`, bloquant sur le **delta** uniquement depuis #1413 : `phpstan-strict-baseline.neon` gele les ~2950 erreurs pre-existantes (voir `api/ARCHITECTURE.md` section "Trajectoire PHPStan").
  - **`composer audit --locked --no-dev`** — job `backend-security` de `tests.yml` (vulnérabilités des dépendances PHP de production, bloquant) + `composer outdated --direct` informatif non bloquant.
  - "level max" est donc bien vérifié en CI sur le périmètre des fichiers modifiés ; le niveau 8 bloquant-sur-delta reste l'état réel le plus proche d'un full-scan strict.
