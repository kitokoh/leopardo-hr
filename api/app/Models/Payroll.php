...
class Payroll extends Model
{
    ...
    public function validator(): BelongsTo { return $this->belongsTo(Employee::class, 'validated_by'); }

    public function scopeDraft(Builder $q): Builder { return $q->where('status', 'draft'); }
    public function scopeValidated(Builder $q): Builder { return $q->where('status', 'validated'); }
    public function scopeForPeriod(Builder $q, int $month, int $year): Builder { return $q->where('period_month', $month)->where('period_year', $year); }
