...
class DepartmentController extends Controller
{
    ...
    {
        return ['id' => $d->id, 'name' => $d->name, 'manager_id' => $d->manager_id, 'manager' => $d->relationLoaded('manager') && $d->manager ? ['id' => $d->manager->id, 'first_name' => $d->manager->first_name, 'last_name' => $d->manager->last_name] : null, 'positions' => $d->relationLoaded('positions') ? $d->positions->map(fn ($p) => ['id' => $p->id, 'name' => $p->name]) : null, 'created_at' => $d->created_at?->toIso8601String()];
    }
}
