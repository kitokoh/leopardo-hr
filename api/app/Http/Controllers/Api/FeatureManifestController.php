...
class FeatureManifestController extends Controller
{
    ...
    {
        $filtered = array_filter($features, function ($feature) use ($user) {
            return $this->userHasFeaturePermissions($user, $feature['permissions'] ?? []);
        });

        // Utiliser array_values pour réindexer le tableau et garantir une sérialisation JSON en tant qu'array
