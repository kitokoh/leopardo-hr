...
class EdgeLicense extends Model
{
    ...
    /**
     * Champs sensibles jamais sérialisés par défaut (#4687) :
     * - license_key : identifiant unique de licence (credential)
     * - signed_payload : blob JWT signé (clé de licence offline)
     * Les flux orientés nœud (enrôlement store(), renouvellement
     * issueLicense()) les exposent explicitement via makeVisible() —
     * aucun autre endpoint ne doit les servir.
     */
    protected $hidden = [
        'license_key',
        'signed_payload',
