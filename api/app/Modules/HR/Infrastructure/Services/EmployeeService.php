...
class EmployeeService
{
    ...
    public function create(CreateEmployeeDTO $dto, ?Employee $actor = null): Employee
    {
        ...
        // Issue #4307 : role/manager_role/status/company_id sont non mass-assignables
        // (durcissement #3677) — les passer dans create() les ferait silencieusement
        // perdre (role null → TypeError EmployeeResource, company_id null → hors tenant).
        // On crée avec les clés fillable puis on pose les clés sensibles explicitement
        // (pattern #3677/#4151) : l'acteur est déjà autorisé (policy + FormRequest).
        $employee = Employee::query()->create(Arr::except($payload, ['role', 'manager_role', 'status', 'company_id']));
        $employee->forceFill([
    ...
    public function update(Employee $actor, Employee $employee, UpdateEmployeeDTO $dto): Employee
    {
        ...
        // Issue #4307 : role/manager_role/status non mass-assignables (#3677) —
        // fill() les écarterait silencieusement (revoke rh jamais persisté).
        $sensitiveKeys = ['role', 'manager_role', 'status'];
