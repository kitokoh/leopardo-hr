...
/**
 * RESTO-304 (#6185) — Validation stricte de création d'un horaire restaurant.
 *
 * Règles applicatives (après validation de base) : un jour ouvert
 * (`is_closed` absent ou false) exige `opens_at` et `closes_at`, et la
 * fermeture doit être strictement postérieure à l'ouverture (même sémantique
 * que la règle native `after:opens_at`, appliquée ici à des heures `H:i`
 * sans date). Un jour fermé (`is_closed` true) ignore les heures.
 */
class StoreRestaurantHourRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true; // RestaurantHourPolicy::create() tranche l'autorisation
    }
