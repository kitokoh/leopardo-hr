...
{
    protected $signature = 'leopardo:restaurant:stock-alerts {company? : UUID de la company a scanner (defaut : toutes)}';
    ...
    public function handle(TenantManager $tenants, StockAlertService $alerts): int
    {
        ...
        if ($companyId !== null) {
            ...
            $this->info("Alertes publiees pour {$company->id} : {$count}.");
        ...
        $this->info("Alertes publiees pour {$companies->count()} tenants : {$total} au total.");
