...
class CompanyProvisioningService
{
    ...
    public function provisionSharedCompany(array $payload, SuperAdmin $superAdmin): array
    {
        ...
        try {
            ...
            // #3811 : deux provisionnements concurrents peuvent choisir le même
            // slug (le while-exists n'est pas atomique) — l'unique companies.slug
            // tranche : 422 explicite au lieu d'un 500 SQL brut.
            if ($e->getCode() === '23505') {
                throw ValidationException::withMessages([
                    'name' => __('errors.COMPANY_SLUG_CONFLICT'),
                ]);
            }
