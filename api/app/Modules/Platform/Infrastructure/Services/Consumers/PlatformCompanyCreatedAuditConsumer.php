...
/**
 * #5866 — Consommateur `platform.company.created` (MAT-008).
 *
 * Trace la création d'un tenant dans la piste d'audit (action
 * `outbox.company_created`, auditable = événement outbox) — preuve de
 * délivrance de l'outbox et horizon de support. Idempotent : l'écriture est
 * sautée si un audit porte déjà le même `event_id` (métadonnée), le rejeu
 * ne duplique jamais.
 */
final class PlatformCompanyCreatedAuditConsumer implements PlatformOutboxConsumer
{
    public const EVENT_TYPE = 'platform.company.created';

    public const AUDIT_ACTION = 'outbox.company_created';
    ...
    public function handle(array $payload): void
    {
        ...
        AuditLog::create([
            ...
            'old_values' => null,
            'new_values' => ['company_name' => $payload['company_name'] ?? null],
            'metadata' => [
                'event_id' => $eventId,
                'outbox' => true,
            ],
