...
/**
 * #5591 — entrées de travail (work inputs) de la paie : agrégats
 * attendance_logs (heures sup, jours présents) + congés approuvés
 * (payés / non payés), avec repli par-employé sur échec batch.
 *
 * Extraites de `PayrollCalculator` (1 676 lignes) pour isoler le concern
 * « requêtes d'agrégation » du calcul pur — comportement identique,
 * couvert par les 448 tests Feature Payroll (dont les golden tests).
 */
final class PayrollWorkInputs
{
    /**
     * Entrées de travail d'un employé pour un run.
     *
     * @param  array{overtime_hours?: float}|null  $attendanceAgg  agrégats batch (#2687)
     * @param  array{paid_leave_days?: float, unpaid_leave_days?: float}|null  $leaveAgg  agrégats batch (#2687)
     ...
     */
    public function aggregate(PayrollRun $run, Collection $employees): array
    {
        $attendance = [];
        $leave = [];
    ...
    /**
     * Jours de congés approuvés (payés ou non) d'un employé sur la période,
     * au prorata de l'intersection (issue #2672 — pas de double déduction
     * sur les absences chevauchantes).
     */
