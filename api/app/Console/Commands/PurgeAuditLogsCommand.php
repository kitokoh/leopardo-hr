    ...
    public function handle(TenantManager $tenantManager): int
    {
        ...
        foreach ($companies as $company) {
            [$deleted, $kept] = $tenantManager->withinTenant($company, function () use ($company, $globalMonths, $dryRun): array {
                ...
                $query = AuditLog::query()->crossTenantForSystemTask('purge rétention RGPD par schéma ambiant, logs plateforme company_id NULL inclus (#7711, #7960)')->where('created_at', '<', $cutoff);
