...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('features', function (Blueprint $table) {
            $table->increments('id');
            $table->string('company_id')->nullable()->index();         // NULL en mode schema isolé
            ...
            $table->json('http_methods');                          // Méthodes HTTP supportées ['GET', 'POST', etc.]
            $table->json('parameters');                            // Schéma des paramètres d'entrée
            $table->json('response_schema');                       // Schéma de la réponse API

            // ── Sécurité et permissions ─────────────────────────────────────
            $table->json('permissions');                           // Permissions requises ['employees.view', etc.]
            ...
            $table->json('metadata');                              // Métadonnées pour génération UI mobile
        ...
        // ── Commentaires pour documentation (PostgreSQL seulement) ─────────────────────────────────
