...
class BillingDelinquencyPolicyTest extends TestCase
{
    ...
    public function test_transition_matrix_is_enforced(): void
    {
        ...
        self::assertTrue(SubscriptionStatus::Expired->canTransitionTo(SubscriptionStatus::Cancelled));
        self::assertFalse(SubscriptionStatus::Expired->canTransitionTo(SubscriptionStatus::PastDue));
        self::assertTrue(SubscriptionStatus::PastDue->canTransitionTo(SubscriptionStatus::Cancelled));
        // cancelled n'est PAS terminal : la réactivation (renew) est légale,
        // mais une souscription résiliée ne peut ni expirer ni retomber en past_due.
    ...
    public function test_invalid_transition_throws(): void
    {
        ...
        $subscription = $this->subscription($company, SubscriptionStatus::Trial->value);

        try {
            // trial ne peut pas sauter en past_due (la politique de recouvrement
            // passe par trial → expired → active).
