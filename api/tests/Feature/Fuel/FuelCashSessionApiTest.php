...
class FuelCashSessionApiTest extends TestCase
{
    ...
    {
        $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 0])->assertStatus(401);
    }
    ...
    public function test_operator_opens_session(): void
    {
        ...
        $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 5000])
            ->assertStatus(201)
    ...
    public function test_operator_adds_movements_and_closes_with_server_computed_variance(): void
    {
        ...
        $sessionId = $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 1000])
            ->assertStatus(201)
            ->json('data.id');

        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/movements", [
            'type' => 'in',
            'amount' => 500,
        ...
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/close", ['closing_balance' => 1250])
            ->assertStatus(200)
    ...
    public function test_close_is_idempotent(): void
    {
        ...
        /** @var string $sessionId */
        $sessionId = $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 0])
            ->json('data.id');

        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/close", ['closing_balance' => 800])
            ->assertStatus(200)
        ...
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/close", ['closing_balance' => 99999])
            ->assertStatus(200)
    ...
    public function test_approval_requires_manager_and_closed_session(): void
    {
        ...
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/approve")
            ->assertStatus(403);
        ...
        Sanctum::actingAs($operator);
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/close", ['closing_balance' => 300])
            ->assertStatus(200);
    ...
    public function test_operator_cannot_touch_another_operators_session(): void
    {
        ...
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/close", ['closing_balance' => 10])
            ->assertStatus(403);
        $this->getJson("/api/v1/fuel/cash-sessions/{$sessionId}")->assertStatus(403);
    }
    ...
    public function test_cross_tenant_session_is_404(): void
    {
        ...
        $this->getJson("/api/v1/fuel/cash-sessions/{$sessionId}")->assertStatus(404);
        $this->postJson("/api/v1/fuel/cash-sessions/{$sessionId}/approve")->assertStatus(404);
    }
    ...
    public function test_manager_lists_sessions_and_operator_self_service(): void
    {
        ...
        Sanctum::actingAs($operatorA);
        $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 100])->assertStatus(201);
        Sanctum::actingAs($operatorB);
        $this->postJson('/api/v1/fuel/cash-sessions', ['opening_balance' => 200])->assertStatus(201);

        Sanctum::actingAs($operatorA);
        $this->getJson('/api/v1/fuel/me/cash-sessions')
            ->assertStatus(200)
            ->assertJsonCount(1, 'data')
            ->assertJsonPath('data.0.opening_balance', 100);

        Sanctum::actingAs($manager);
        $this->getJson('/api/v1/fuel/cash-sessions')
            ->assertStatus(200)
