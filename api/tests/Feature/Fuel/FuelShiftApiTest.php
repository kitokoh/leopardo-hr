 ...
 * self-service pompiste (/fuel/me/shifts), suppression refusée d'un shift
...
class FuelShiftApiTest extends TestCase
{
    ...
    {
        $this->getJson('/api/v1/fuel/shifts')->assertStatus(401);
        $this->getJson('/api/v1/fuel/me/shifts')->assertStatus(401);
    }
    ...
    public function test_operator_employee_cannot_manage_shifts(): void
    {
        ...
        $this->getJson('/api/v1/fuel/shifts')->assertStatus(403);
        $this->postJson('/api/v1/fuel/shifts', [
            'name' => 'Matin',
            'start_time' => '06:00',
            'end_time' => '14:00',
        ])->assertStatus(403);
    ...
    public function test_manager_creates_and_lists_shift(): void
    {
        ...
        $this->getJson('/api/v1/fuel/shifts')
            ->assertStatus(200)
    ...
    public function test_shift_name_must_be_unique_per_company(): void
    {
        ...
        $this->postJson('/api/v1/fuel/shifts', ['name' => 'Matin', 'start_time' => '06:00', 'end_time' => '14:00'])
            ->assertStatus(201);

        $this->postJson('/api/v1/fuel/shifts', ['name' => 'Matin', 'start_time' => '14:00', 'end_time' => '22:00'])
            ->assertStatus(422)
    ...
    public function test_end_time_must_be_after_start_time(): void
    {
        ...
        $this->postJson('/api/v1/fuel/shifts', ['name' => 'Nuit', 'start_time' => '22:00', 'end_time' => '06:00'])
            ->assertStatus(422)
    ...
    public function test_cross_tenant_shift_access_is_404(): void
    {
        ...
        Sanctum::actingAs($managerB);
        $this->getJson("/api/v1/fuel/shifts/{$shift->id}")->assertStatus(404);
        $this->putJson("/api/v1/fuel/shifts/{$shift->id}", ['name' => 'X'])->assertStatus(404);
        $this->deleteJson("/api/v1/fuel/shifts/{$shift->id}")->assertStatus(404);
    }
    ...
    public function test_manager_assigns_operator_to_shift(): void
    {
        ...
        $this->postJson("/api/v1/fuel/shifts/{$shift->id}/assignments", [
    ...
    public function test_overlapping_assignment_same_day_is_rejected(): void
    {
        ...
        $this->postJson("/api/v1/fuel/shifts/{$morning->id}/assignments", [
            'employee_id' => $operator->id,
            'assignment_date' => '2026-09-01',
        ])->assertStatus(201);

        // Chevauchement (13:00 < 14:00) → 422 SHIFT_OVERLAP.
        $this->postJson("/api/v1/fuel/shifts/{$afternoon->id}/assignments", [
            'employee_id' => $operator->id,
            'assignment_date' => '2026-09-01',
        ])
            ->assertStatus(422)
        ...
        $this->postJson("/api/v1/fuel/shifts/{$inactive->id}/assignments", [
            'employee_id' => $operator->id,
            'assignment_date' => '2026-09-01',
        ])
            ->assertStatus(422)
    ...
    public function test_operator_sees_only_own_assignments_via_me_shifts(): void
    {
        ...
        $this->getJson('/api/v1/fuel/me/shifts?date_from=2026-09-01&date_to=2026-09-30')
            ->assertStatus(200)
            ->assertJsonCount(1, 'data')
    ...
    public function test_delete_shift_with_assignments_is_rejected(): void
    {
        ...
        $this->deleteJson("/api/v1/fuel/shifts/{$shift->id}")
            ->assertStatus(422)
        ...
        $assignment = FuelShiftAssignment::query()
            ...
            ->firstOrFail();
        $this->deleteJson("/api/v1/fuel/shift-assignments/{$assignment->id}")->assertStatus(200);

        $this->deleteJson("/api/v1/fuel/shifts/{$shift->id}")->assertStatus(200);
    }
    ...
    public function test_manager_cancels_assignment(): void
    {
        ...
        $assignmentId = $this->postJson("/api/v1/fuel/shifts/{$shift->id}/assignments", [
            'employee_id' => $operator->id,
            'assignment_date' => '2026-09-01',
        ])->assertStatus(201)->json('data.id');

        $this->deleteJson("/api/v1/fuel/shift-assignments/{$assignmentId}")
            ->assertStatus(200)
