...
/**
 * DELIVERY-101 (#6282) — Gate du module Delivery (BC-26 DELIVERY).
 *
 * La route de smoke test `GET /api/v1/delivery/ping` doit être :
 *   - inaccessible sans authentification (401) ;
 *   - refusée quand le feature flag `delivery` est inactif (403) ;
 *   - accessible quand le flag est actif (200 {status: ok}).
 */
class DeliveryFeatureFlagTest extends TestCase
{
    use RefreshTenantDatabase;

    public function test_ping_requires_authentication(): void
    {
        $this->getJson('/api/v1/delivery/ping')
            ->assertStatus(401);
    }
