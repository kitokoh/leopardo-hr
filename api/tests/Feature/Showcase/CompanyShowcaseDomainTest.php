 ...
 * une seule vitrine par tenant, unicité globale du slug, scope tenant,
 * feature flag `company_showcase` et RBAC deny-by-default (gestion réservée
 * principal/rh du tenant).
 */
class CompanyShowcaseDomainTest extends TestCase
{
    ...
    public function test_showcase_is_created_with_company_id_not_null_and_defaults(): void
    {
        $showcase = $this->showcase($this->companyA);

        $this->assertSame($this->companyA->id, $showcase->company_id);
        $this->assertSame(CompanyShowcaseStatus::Draft, $showcase->status);
        $this->assertSame('default', $showcase->theme);
    ...
    public function test_company_can_have_only_one_showcase(): void
    {
        $this->showcase($this->companyA, 'acme-industries');

        // Une vitrine chez un AUTRE tenant → autorisé (isolation).
        $this->showcase($this->companyB, 'autre-tenant');

        // Une seconde vitrine chez le MÊME tenant → contrainte unique (savepoint #4978).
        $this->expectException(QueryException::class);
        DB::transaction(function (): void {
            $this->showcase($this->companyA, 'acme-bis');
        });
    }
    ...
    public function test_slug_is_unique_globally(): void
    {
        ...
        // Même slug chez un AUTRE tenant → refusé (slug = clé publique globale).
        $this->expectException(QueryException::class);
        DB::transaction(function (): void {
    ...
    public function test_showcase_policy_denies_cross_tenant_management(): void
    {
        ...
        // … mais pas celle de B (isolation tenant, 404/403 côté API plus tard).
