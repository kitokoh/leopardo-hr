...
## [Unreleased]
- **feat(attendance): ADR-0016 Phase 5 — nettoyage final : module SmartAttendance supprimé, surface API unifiée sur `/attendance/*` (Closes #5356).** Module `api/app/Modules/SmartAttendance/` supprimé (16 fichiers : shims de re-export `@deprecated`, README, provider) ; contrôleurs géo (`AttendanceModeController`, `GeoAttendanceController`, `GeoSessionController`) + 5 FormRequests déplacés dans `App\Modules\Attendance\Interfaces\Api\V1\*` (comportement identique) ; alias de routes `/api/v1/smart-attendance/*` purgés (`smart_attendance.php` + `SmartAttendanceServiceProvider` + enregistrement `bootstrap/providers.php`) ; commande dépréciée `smart-attendance:auto-close` supprimée — scénarios (sessions orphelines, dry-run, filtre `--company` multi-tenant) portés dans `AutoCloseAttendanceSessionsTest` (`attendance:auto-close --sessions-only`). OpenAPI : 11 opérations `/smart-attendance/*` retirées + schéma `SmartAttendanceDashboard` renommé `AttendanceGeoDashboard` (788 opérations), miroir + SDK JS/Python régénérés. Front web : pages `(dashboard)/smart-attendance/*` migrées sous `/attendance/geo/*` (appels API sur `/attendance/*`), guards `middleware`/`protected-prefixes`/`client-features`/`sw.js` purgés. Contrat mobile vérifié : les 3 apps appellent exclusivement `/attendance/*` (navigation interne Flutter inchangée, aucun appel API legacy). Garde CI `dev-hub/tools/check-no-smartattendance.sh` branchée dans Architecture Quality (zéro import `App\Modules\SmartAttendance\*`, zéro chemin `/smart-attendance` dans API/OpenAPI/SDK/web). Tests : 8 fichiers migrés vers `/attendance/*`, `GeoRoutesMigrationTest` inverse les assertions d'alias (404) ; baselines PHPStan purgées (41 entrées) ; tracker `POINTAGE_100PCT.md` + CHANGELOG à jour. Bonus réparation main : openapi.yaml corrompu depuis #5229 (bloc `/attendance/mode-settings` épissé + chemin `/accounting/documents/{document}/payments` dupliqué — lint redocly rouge sur main) reconstruit depuis le dernier état valide (5cd78d4) + 4 opérations paiements réintégrées proprement ; parseur `check-openapi-route-coverage.py` corrigé (contexte hérité dédupliqué pour le double `require accounting.php` dans `api.php`, chemins relatifs joints avec `/`) — **couverture 790/790 routes, 0 drift**.
