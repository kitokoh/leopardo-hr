...
- **Attendance unified calculation + day closure (issue #5265)**: `AttendanceHoursCalculator` (pure: late assessment/tolerance, worked hours − breaks, overtime threshold + `overtime` work-type + non-work types #2686) — single source of truth replacing the 4 duplicated blocks in `AttendanceService` and the divergent calc in `ApproveGeoSession` (geo now deducts schedule breaks; parity proven by unchanged suites). `attendance_day_closures` (additive tenant migration `2026_08_24_000003`): daily lock per (company, employee, date), idempotent `lockDay`, distinct `validateDay`, `unlockDay`, `listFor`; guard 409 `ATTENDANCE_DAY_CLOSED` on check-in/check-out/import/geo approval; endpoints `GET/POST /api/v1/attendance/day-closures` + `POST /{id}/validate` + `DELETE /{id}` (RBAC `api.manager:rh,principal`, tenant fail-closed); i18n ×4 (`errors.ATTENDANCE_DAY_CLOSED`, 216 keys parity); OpenAPI 4 operations + `AttendanceDayClosure` schema (622 paths, mirror + SDK regenerated); tests `AttendanceHoursCalculatorTest` (hand-computed golden) + `AttendanceDayClosureTest` (13 scenarios).
