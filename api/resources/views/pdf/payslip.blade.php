@php
    // Issue #5242 — rendu RTL : police arabe (Almarai) + shaping des
    // chaînes arabes (formes contextuelles + inversion) pour dompdf.
    $rtl = \App\Support\I18nCatalog::isRtl(app()->getLocale());
...
<head>
    <meta charset="utf-8">
    <style>
        body { font-family: {{ $rtl ? 'Almarai' : 'DejaVu Sans' }}, 'DejaVu Sans', sans-serif; font-size: 11px; color: #111; margin: 30px; }
        ...
        @if($rtl) th, td { text-align: right; } .info-col { direction: rtl; } @endif
...
<body>
    ...
    <div style="text-align: center; font-size: 13px; font-weight: bold; color: #b45309; background: #fef3c7; border: 2px solid #b45309; padding: 8px; margin-bottom: 12px;">
        @if(!empty($originalSlipId))
            {{ $t('pdf.payslip_regularization_banner_slip', ['slip' => $originalSlipId]) }}
        @else
            {{ $t('pdf.payslip_regularization_banner', ['run' => $originalRunId ?? '']) }}
        @endif
    </div>
    @endif
    <div class="period">{{ $t('pdf.period') }} : {{ $slip->period_start->format('d/m/Y') }} — {{ $slip->period_end->format('d/m/Y') }}</div>

    <div class="info-grid">
        <div class="info-col">
            <div class="info-label">{{ $t('pdf.employee') }}</div>
            <div class="info-value">{{ $employee->first_name ?? '' }} {{ $employee->last_name ?? '' }}</div>
            <div class="info-label">{{ $t('pdf.payslip_matricule') }}</div>
            <div class="info-value">#{{ $employee->id }}</div>
        </div>
        <div class="info-col">
            <div class="info-label">{{ $t('pdf.payslip_worked_days') }}</div>
            <div class="info-value">{{ $slip->actual_days_worked ?? $slip->working_days ?? 22 }} / {{ $slip->working_days ?? 22 }}</div>
            <div class="info-label">{{ $t('pdf.overtime_hours') }}</div>
            <div class="info-value">{{ $slip->overtime_hours ?? 0 }}h</div>
        </div>
    </div>

    {{-- Earnings --}}
    <div class="section-title">{{ $t('pdf.payslip_earnings_section') }}</div>
    <table>
        <thead>
            <tr>
                <th>{{ $t('pdf.payslip_column_label') }}</th>
                <th>{{ $t('pdf.payslip_column_base') }}</th>
                <th>{{ $t('pdf.payslip_column_rate') }}</th>
                <th class="amount">{{ $t('pdf.payslip_column_amount') }}</th>
            </tr>
        </thead>
        <tbody>
    ...
    {{-- Deductions --}}
    <div class="section-title">{{ $t('pdf.payslip_deductions_section') }}</div>
    <table>
        <thead>
            <tr>
                <th>{{ $t('pdf.payslip_column_label') }}</th>
    ...
    <div class="section-title">{{ $t('pdf.payslip_employer_contributions_section') }}</div>
    <table>
        <tbody>
    ...
    <table style="margin-top: 10px;">
        <tr class="total-row">
            <td>{{ $t('pdf.payslip_gross_salary') }}</td>
            <td class="amount">{{ number_format($slip->gross_salary, 2, ',', ' ') }} {{ $currency }}</td>
        </tr>
        <tr class="total-row">
            <td>{{ $t('pdf.payslip_total_deductions') }}</td>
            <td class="amount">{{ number_format($slip->total_deductions, 2, ',', ' ') }} {{ $currency }}</td>
        </tr>
    </table>

    @if(($annualCumuls['gross'] ?? 0) > 0)
    <div class="section-title">{{ $t('pdf.payslip_annual_cumuls') }}</div>
    <table>
        <tr>
            <th>{{ $t('pdf.payslip_gross_salary') }}</th>
            <th>{{ $t('pdf.payslip_total_deductions') }}</th>
            <th>{{ $t('pdf.payslip_net_to_pay') }}</th>
        </tr>
        <tr>
    ...
    <div class="net-box">
        <div class="net-label">{{ $t('pdf.payslip_net_to_pay') }}</div>
        <div class="net-amount">{{ number_format($slip->net_salary, 2, ',', ' ') }} {{ $currency }}</div>
    </div>

    <div class="footer">
        <div>{{ $t('pdf.payslip_generated_on', ['date' => now()->format('d/m/Y H:i')]) }} — Leopardo RH</div>
        <div>{{ $t('pdf.payslip_official_notice') }}</div>
