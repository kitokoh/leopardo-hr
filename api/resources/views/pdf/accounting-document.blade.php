...
<head>
    <meta charset="utf-8">
    <style>
        body { font-family: DejaVu Sans, sans-serif; color: #172033; font-size: 11px; margin: 0; }
...
<body>
    <div class="header">
        ...
        <div class="doc-meta">
            <div class="type">{{ $document_type_label }}</div>
            <div><span class="badge">{{ $status_label }}</span></div>
            <div><span class="muted">{{ __('accounting.number') }}:</span> {{ $document->number }}</div>
            <div><span class="muted">{{ __('accounting.issue_date') }}:</span> {{ $document->issue_date?->format('d/m/Y') }}</div>
            @if ($document->due_date)
                <div><span class="muted">{{ __('accounting.due_date') }}:</span> {{ $document->due_date->format('d/m/Y') }}</div>
            @endif
            @if ($document->delivery_date)
                <div><span class="muted">{{ __('accounting.delivery_date') }}:</span> {{ $document->delivery_date->format('d/m/Y') }}</div>
            @endif
        </div>
    </div>
    ...
    <div class="parties">
        <div>
            <div class="muted">{{ __('accounting.from') }}</div>
        ...
        @if ($contact)
            <div class="to">
                <div class="muted">{{ __('accounting.to') }}</div>
                <div><strong>{{ $contact->name }}</strong></div>
                @if (! empty($contact->tax_id))
                    <div>{{ __('accounting.nif') }}: {{ $contact->tax_id }}</div>
                @endif
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="num">{{ __('accounting.quantity') }}</th>
                <th class="num">{{ __('accounting.unit_price') }}</th>
                <th class="num">{{ __('accounting.discount') }}</th>
                <th class="num">{{ __('accounting.amount') }}</th>
            </tr>
        </thead>
        <tbody>
            ...
            @empty
                <tr><td colspan="5" class="muted">{{ __('accounting.no_lines') }}</td></tr>
            @endforelse
        </tbody>
    </table>
    ...
    @if ($legal_mentions)
        <div class="note">
            <strong>{{ __('accounting.legal_mentions') }}</strong><br>
            {{ $legal_mentions }}
        </div>
    @endif

    <div class="footer">
        {{ $company->name }} — {{ $document_type_label }} {{ $document->number }}
    </div>
</body>
</html>
