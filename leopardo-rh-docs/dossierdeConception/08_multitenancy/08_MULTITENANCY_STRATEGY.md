...
class TenantMigrationService
{
    public function migrateSharedToSchema(Company $company): void
    {
        ...
        // 2. Appliquer les migrations tenant dans ce schéma
        DB::statement("SET search_path TO {$schemaName}, public");
        Artisan::call('migrate', ['--path' => 'database/migrations/tenant', '--force' => true]);

        // 3. Copier les données (en batchs pour éviter timeouts)
        $tables = ['employees', 'attendance_logs', 'absences', 'payrolls', ...];
        ...
        // 5. Supprimer les données de shared (RGPD)
        foreach (array_reverse($tables) as $table) {
            DB::table("shared_tenants.{$table}")->where('company_id', $company->id)->delete();
        }
    }
}
```
